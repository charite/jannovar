...
public class BedFileAnnotator implements Closeable {
  ...
  public BedFileAnnotator(BedAnnotationOptions options) {
    ...
    try {
      this.reader =
          new TabixFeatureReader<>(
              featureFile.getAbsolutePath().toString(),
              featureFile.getAbsolutePath().toString() + ".tbi",
              new BEDCodec());
    } catch (IOException e) {
      ...
      if (options.getColNo() < 0) {
        vcfHeader.addMetaDataLine(
            new VCFInfoHeaderLine(
                options.getInfoField(), 0, VCFHeaderLineType.Flag, options.getDescription()));
      } else {
        vcfHeader.addMetaDataLine(
            new VCFInfoHeaderLine(
                options.getInfoField(),
  ...
  public VariantContext annotateVariantContext(VariantContext vc) {
    ...
    try {
      ...
      for (BEDFeature bedFeature :
          reader.query(vc.getContig(), vc.getStart() - 1, vc.getEnd() + 1)) {
        final Interval bedItv =
            new Interval(bedFeature.getContig(), bedFeature.getStart(), bedFeature.getEnd());
      ...
      throw new RuntimeException(
          "Could not query " + vc.getContig() + ":" + vc.getStart() + "-" + vc.getEnd(), e);
    }
