...
public class ProjectTranscriptToChromosome extends JannovarAnnotationCommand {
  ...
  private VariantContextWriter openOutputFile() {
    ...
    header.addMetaDataLine(
        new VCFInfoHeaderLine("ERROR_MESSAGE", 1, VCFHeaderLineType.String, "Error message"));
    header.addMetaDataLine(
  ...
  private VariantContext buildErrorVariantContext(String origString, String message) {
    ...
    return new VariantContextBuilder()
        .loc("1", 1, 1)
        .alleles(Lists.newArrayList(alleleRef, alleleAlt))
        .filter("PARSE_ERROR")
        .attribute("ORIG_VAR", urlEncode(origString))
        .attribute("ERROR_MESSAGE", urlEncode(message))
        .make();
  }
  ...
  private void processFile(VariantContextWriter writer) {
    try (BufferedReader br =
        new BufferedReader(new FileReader(new File(options.getPathInputText())))) {
      String line;
      while ((line = br.readLine()) != null) {
        ...
        try {
          ...
          if (!(rawVar instanceof SingleAlleleNucleotideVariant)) {
            writer.add(
                buildErrorVariantContext(word, "More than one allele in nucleotide variant"));
            continue;
          }
  ...
  private void writeVariant(VariantContextWriter writer, GenomeVariant genomeVar) {
    ...
    if (genomeVar.getRef().isEmpty() || genomeVar.getAlt().isEmpty()) {
      ...
      String left =
          fasta
              .getSubsequenceAt(nameInFasta, genomeVar.getPos(), genomeVar.getPos())
              .getBaseString();
    ...
    builder
        .chr(genomeVar.getChrName())
        .start(genomeVar.getPos() + shift + 1)
        .computeEndFromAlleles(alleles, genomeVar.getPos() + shift + 1)
        .alleles(alleles);
  ...
  private GenomeVariant translate(SingleAlleleNucleotideVariant rawVar) {
    try {
      return translator.translateNucleotideVariantToGenomeVariant(
          (SingleAlleleNucleotideVariant) rawVar, true);
