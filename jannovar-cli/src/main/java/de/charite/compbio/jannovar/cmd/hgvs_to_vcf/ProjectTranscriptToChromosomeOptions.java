...
public class ProjectTranscriptToChromosomeOptions extends JannovarAnnotationOptions {
  ...
  public static void setupParser(Subparsers subParsers) {
    BiFunction<String[], Namespace, ProjectTranscriptToChromosome> handler =
        (argv, args) -> {
          try {
    ...
    Subparser subParser =
        subParsers
            .addParser("hgvs-to-vcf", true)
            .help("project transcript-level to chromosome-level changes")
    ...
    requiredGroup
        .addArgument("-r", "--reference-fasta")
        .help("Path to reference FASTA file")
        .required(true);
    requiredGroup.addArgument("-d", "--database").help("Path to database .ser file").required(true);
    requiredGroup
        .addArgument("-i", "--input-txt")
        .help("Input file with HGVS transcript-level changes, line-by-line")
        .required(true);
    requiredGroup
        .addArgument("-o", "--output-vcf")
        .help("Output VCF file with chromosome-level changes")
        .required(true);
  ...
  public String toString() {
    return "ProjectTranscriptToChromosomeOptions [pathInputText="
        + pathInputText
        + ", pathOutputVCF="
        + pathOutputVCF
        + ", pathReferenceFASTA="
