...
public class JannovarAnnotateVCFTest {
  ...
  // Test on small.vcf (with default settings) and compare with the prepared gold-standard
  // small.jv.vcf
  @Test
  public void testOnSmallExample() throws JannovarException, URISyntaxException, IOException {
    ...
    String[] argv =
        new String[] {
          "annotate-vcf",
          "-o",
          outFolder.toString() + "/small.jv.vcf",
          "-d",
          pathToSmallSer,
          "-i",
          inputFilePath
    ...
    final File expectedFile =
        new File(this.getClass().getResource("/small.jv.vcf").toURI().getPath());
    final String expected = Files.asCharSource(expectedFile, Charsets.UTF_8).read();
    final String actual =
        Files.asCharSource(f, Charsets.UTF_8)
            .read()
            .replaceAll("##jannovarCommand.*", "##jannovarCommand")
            .replaceAll("##jannovarVersion.*", "##jannovarVersion");
    Assert.assertEquals(expected, actual);
  }

  // Test on semicolons.vcf. This file contains trailing semicolons at the end of the INFO and
  // FILTER columns.
  // Previous versions of Jannovar directly used the HTSJDK, interpreted this as empty entries and
  // moved the semicolon
  ...
  public void testOnTrailingSemicolons() throws JannovarException, URISyntaxException, IOException {
    ...
    String[] argv =
        new String[] {
          "annotate-vcf",
          "-o",
          outFolder.toString() + "/semicolons.jv.vcf",
          "-d",
          pathToSmallSer,
          "-i",
          inputFilePath
    ...
    final File expectedFile =
        new File(this.getClass().getResource("/semicolons.jv.vcf").toURI().getPath());
    final String expected = Files.asCharSource(expectedFile, Charsets.UTF_8).read();
    final String actual =
        Files.asCharSource(f, Charsets.UTF_8)
            .read()
