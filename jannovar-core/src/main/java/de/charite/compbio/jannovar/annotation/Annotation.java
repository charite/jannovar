 ...
 *
 * <p>Implements the {@link VariantDescription} interface for quicker access to the variant
...
public final class Annotation implements VariantDescription, Comparable<Annotation> {
  ...
  private static final ImmutableSet<VariantEffect> EMPTY_VARIANT_EFFECTS =
      Sets.immutableEnumSet(EnumSet.noneOf(VariantEffect.class));
  private static final ImmutableSet<AnnotationMessage> EMPTY_ANNOTATION_MESSAGES =
      Sets.immutableEnumSet(EnumSet.noneOf(AnnotationMessage.class));

  /**
   * This line is added to the output of a VCF file annotated by Jannovar and describes the new
   * field for the INFO section entitled EFFECT, which decribes the effects of variants
  ...
  /**
   * This line is added to the output of a VCF file annotated by Jannovar and describes the new
   * field for the INFO section entitled HGVS, which provides the HGVS encoded variant corresponding
   * to the chromosomal variant in the original VCF file.
   */
  ...
  /**
   * CDS-level {@link NucleotideChange} for coding transcripts (to be prependend with "c.") and
   * transcript level for non-coding transcripts (to be prepended with "n.")
   */
   ...
   *
   * <p>The constructor will sort <code>effects</code> by pathogenicity before storing.
   *
   * @param change the annotated {@link GenomeVariant}
   * @param transcript transcript for this annotation
   * @param effects type of the variants
   * @param annoLoc location of the variant
   * @param genomicNTChange ghromosome/genome-level change, to be prepended with "g."
   * @param cdsNTChange CDS-level {@link NucleotideChange}
   * @param proteinChange predicted {@link ProteinChange}
   */
  public Annotation(
   ...
   * @param transcript transcript for this annotation
   * @param change the annotated {@link GenomeVariant}
   * @param varTypes type of the variants
   * @param annoLoc location of the variant
   * @param genomicNTChange ghromosome/genome-level change, to be prepended with "g."
   * @param cdsNTChange CDS-level {@link NucleotideChange}
   * @param proteinChange {@link ProteinChange} with a predicted protein change
   * @param messages {@link Collection} of {@link AnnotationMessage} objects
   */
  public Annotation(
      TranscriptModel transcript,
      GenomeVariant change,
  ...
  /**
   * @return location of the annotation, <code>null</code> if not even nearby a {@link
   *     TranscriptModel}
   */
  ...
  /**
   * @return {@link NucleotideChange} with changes on the CDS level for coding transcripts and on
   *     the transcript level otherwise, null if the change does not affect any transcript
   */
  ...
  /**
   * @return CDS nucleotide change String, including the "p." prefix or the empty string if there is
   *     no annotation.
   */
   ...
   * @return protein change String, including the "p." prefix or the empty string if there is no
   *     annotation.
   */
  ...
  /**
   * @return the transcript, <code>null</code> for {@link VariantEffect#INTERGENIC_VARIANT}
   *     annotations
   */
   ...
   *
   * <p>The <code>ALT</code> allele has to be given to this function since we trim away at least the
   * first base of <code>REF</code>/<code>ALT</code>.
   *
   * @param alt alt allele
   * @param code Three or one letter amino acid code
   * @param escape whether or not to escape the invalid VCF characters, e.g. <code>'='</code>.
  ...
  /**
   * Forward to {@link #toVCFAnnoString(String, boolean, AminoAcidCode) toVCFAnnoString(alt, true,
   * code)}.
   *
   ...
   *
   * <p>If this annotation does not have a symbol (e.g., for an intergenic annotation) then just
   * return the annotation string, e.g., <code>"KIAA1751:uc001aim.1:exon18:c.T2287C:p.X763Q"</code>.
   *
   * @param code Three ore one letter amino acid code
  ...
  public String getSymbolAndAnnotation(AminoAcidCode code) {
    ...
    return Joiner.on(":")
        .skipNulls()
        .join(
            transcript.getGeneSymbol(),
            transcript.getAccession(),
            getCDSNTChangeStr(),
            getProteinChangeStr(code));
  }
  ...
  public String toString() {
    return "Annotation [change="
        ...
        + cdsNTChange
        + ", proteinChange="
        + (proteinChange == null ? "." : proteinChange.toHGVSString())
        + ", transcript.getAccession()="
        + transcript.getAccession()
        + "]";
  }
