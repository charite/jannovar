 ...
 *
 * <p>Simplifies building of <code>Object</code> arrays that can then be joined using {@link
 * Joiner}.
 */
class VCFAnnotationData {
  ...
  public void setAnnoLoc(AnnotationLocation annoLoc) {
    ...
    try {
      this.cdsPos =
          projector.projectGenomeToCDSPosition(projector.transcriptToGenomePos(txPos)).getPos();
   ...
   * @return array of objects to be converted to string and joined, the alternative allele is given
   *     by
   */
  public Object[] toArray(String allele, AminoAcidCode code) {
    final Joiner joiner = Joiner.on('&').useForNull("");
    final String effectsString =
        joiner.join(FluentIterable.from(effects).transform(VariantEffect.TO_SO_TERM));
    return new Object[] {
      allele,
      effectsString,
      impact,
      ...
      (proteinChange == null) ? proteinChange : ("p." + proteinChange.toHGVSString(code)),
      getTXPosString(),
      getCDSPosString(),
      getAminoAcidPosString(),
      getDistanceString(),
  ...
  private String escape(String str) {
    // Escaping follows the requirements of (1) VCF 4.2 and (2) the "Variant annotations in VCF
    // format document.
    // We use the strategy of keeping as much as possible reconstructable (bijective mappings, for
    // the
