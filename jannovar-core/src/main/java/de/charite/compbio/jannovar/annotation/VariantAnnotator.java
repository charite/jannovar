...
public class VariantAnnotator {
    ...
    public VariantAnnotations buildAnnotations(GenomeVariant change) throws AnnotationException {
        ...
        if (changeInterval.length() == 0) {
            ...
            qr = chr.getTMIntervalTree().findOverlappingWithInterval(changeInterval.getBeginPos(), changeInterval.getEndPos());
        }
        List<TranscriptModel> candidateTranscripts = new ArrayList<>(qr.getEntries());
        ...
        // transcripts and collect annotations for each (they are collected in annotations).
    ...
    private void buildSVAnnotation(List<Annotation> annotations, GenomeVariant change, TranscriptModel transcript) throws AnnotationException {
        annotations.add(new StructuralVariantAnnotationBuilder(transcript, change).build());
    }

    private void buildNonSVAnnotation(List<Annotation> annotations, GenomeVariant change, TranscriptModel leftNeighbor, TranscriptModel rightNeighbor)
            throws AnnotationException {
        buildNonSVAnnotation(annotations, change, leftNeighbor);
        buildNonSVAnnotation(annotations, change, rightNeighbor);
    }

    private void buildNonSVAnnotation(List<Annotation> annotations, GenomeVariant change, TranscriptModel transcript) throws InvalidGenomeVariant {
        if (transcript != null) {
