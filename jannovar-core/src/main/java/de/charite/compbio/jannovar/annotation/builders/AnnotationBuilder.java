 ...
 *
 * <p>The helpers subclass this class and and call the superclass constructor in their constructors.
 * This initializes the decorators for {@link #transcript} and initializes protected member such as
 * {@link #locAnno}. The annotation building process is greatly simplified by this.
 *
 * <p>The realizing classes then override {@link #build} and implement their annotation building
 * logic there.
 *
 * <p>At the moment, this has package visibility only since it is not clear yet whether and how
 * client code should extend the builder hierarchy.
 *
 * @author <a href="mailto:manuel.holtgrewe@charite.de">Manuel Holtgrewe</a>
 */
abstract class AnnotationBuilder {

  private static final ImmutableSet<VariantEffect> SPLICE_VARIANT_EFFECTS =
      Sets.immutableEnumSet(
          VariantEffect.SPLICE_DONOR_VARIANT,
          VariantEffect.SPLICE_ACCEPTOR_VARIANT,
          VariantEffect.SPLICE_REGION_VARIANT);
  private static final ImmutableSet<VariantEffect> UPSTREAM_GENE_VARIANT =
      Sets.immutableEnumSet(VariantEffect.UPSTREAM_GENE_VARIANT);
  private static final ImmutableSet<VariantEffect> DOWNSTREAM_GENE_VARIANT =
      Sets.immutableEnumSet(VariantEffect.DOWNSTREAM_GENE_VARIANT);
  private static final ImmutableSet<VariantEffect> INTERGENIC_VARIANT =
      Sets.immutableEnumSet(VariantEffect.INTERGENIC_VARIANT);
   ...
   *
   * <p>Note that {@link #change} will be initialized with normalized positions (shifted to the
   * left) if possible.
   *
  ...
  AnnotationBuilder(
    ...
    // Project the change to the same strand as transcript, reverse-complementing the REF/ALT
    // strings.
   ...
   *
   * <p>We can handle these cases quite easily from the location and amino acid change and just have
   * to check the splicing and intronic/exonic cases.
   *
  ...
  protected Annotation buildIntergenicAnnotation() {
    return new Annotation(transcript, change, INTERGENIC_VARIANT, null, null, null, null, messages);
  }
  ...
  private AnnotationLocation buildLocAnno(TranscriptModel transcript, GenomeVariant change) {
    ...
    // System.err.println("PROJECTED CHANGE\t" +
    // projector.projectGenomeToTXInterval(change.getGenomeInterval()));
   ...
   * @param transcript {@link TranscriptModel} to build annotation for
   * @param change {@link GenomeVariant} to build annotation for
   * @return {@link NucleotideRange} describing the CDS-level position of the change (or
   *     transcript-level in the case of non-coding transcripts)
   */
