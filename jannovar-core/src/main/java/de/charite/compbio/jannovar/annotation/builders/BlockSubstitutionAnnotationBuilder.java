...
// TODO(holtgrem): The block substitution protein annotation generation needs some love in the
// corner cases.
...
public final class BlockSubstitutionAnnotationBuilder extends AnnotationBuilder {
  ...
  private static final ImmutableSet<VariantEffect> TRANSCRIPT_ABLATION =
      Sets.immutableEnumSet(VariantEffect.TRANSCRIPT_ABLATION);
  private static final ImmutableSet<VariantEffect> START_LOST =
      Sets.immutableEnumSet(VariantEffect.START_LOST);
   ...
   */
  public BlockSubstitutionAnnotationBuilder(
  ...
  protected NucleotideChange getCDSNTChange() {
    return new NucleotideIndel(
        false,
        ntChangeRange,
        new NucleotideSeqDescription(change.getRef()),
  ...
  private class CDSExonicAnnotationBuilder {
    ...
    public CDSExonicAnnotationBuilder() {
      ...
      // Get the reference change begin position as CDS coordinate, handling introns and positions
      // outside of CDS.
      this.refChangeBeginPos =
      ...
      this.aaChange =
          new AminoAcidChange(
              refChangeBeginPos.getPos() / 3,
              wtAASeq.substring(
                  refChangeBeginPos.getPos() / 3, (refChangeLastPos.getPos() + 1 + 2) / 3),
              varAASeq.substring(
    ...
    private void handleNonFrameShiftCase() {
      // The variant is a non-frameshift block substitution. The substitution could span more than
      // one exon and
      // thus also affect a splice donor or acceptor site, delete a stop codon. Further, the variant
      // might also be
      ...
      final String insertedAAs =
          varAASeq.substring(aaChange.getPos(), aaChange.getPos() + aaChange.getAlt().length());
      final String mutAAFirst = insertedAAs.isEmpty() ? "" : insertedAAs.substring(0, 1);

      // We differentiate the case of replacing a single amino acid and replacing multiple ones.
      // Note that
    ...
    private void handleFrameShiftCase() {
      ...
      if (varAAStopPos >= 0) {
        ...
        varTypes.addAll(
            ImmutableList.of(VariantEffect.FRAMESHIFT_VARIANT, VariantEffect.STOP_LOST));
      }
