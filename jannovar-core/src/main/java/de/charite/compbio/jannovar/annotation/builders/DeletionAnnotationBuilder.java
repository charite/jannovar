...
/**
 * Builds {@link Annotation} objects for the deletion {@link GenomeVariant}s in the given {@link
 * TranscriptModel}.
 *
...
public final class DeletionAnnotationBuilder extends AnnotationBuilder {
  ...
  private Annotation buildFeatureAblationAnnotation() {
    return new Annotation(
        transcript,
        change,
        ImmutableList.of(VariantEffect.TRANSCRIPT_ABLATION),
        locAnno,
        getGenomicNTChange(),
        getCDSNTChange(),
  ...
  private Annotation buildStartLossAnnotation() {
    return new Annotation(
        transcript,
        change,
        ImmutableList.of(VariantEffect.START_LOST),
        locAnno,
        getGenomicNTChange(),
        getCDSNTChange(),
        ProteinMiscChange.build(true, ProteinMiscChangeType.NO_PROTEIN),
  ...
  private class CDSExonicAnnotationBuilder {
    ...
    public CDSExonicAnnotationBuilder() {
      ...
      // Get the change begin position as CDS coordinate, handling introns and positions outside of
      // CDS.
      this.changeBeginPos =
          projector.projectGenomeToCDSPosition(changeInterval.getGenomeBeginPos());
      this.changeLastPos =
          projector.projectGenomeToCDSPosition(changeInterval.getGenomeEndPos().shifted(-1));
    ...
    private void handleNonFrameShiftCase() {
      // The variant is a non-frameshift deletion. The deletion could span more than one exon and
      // thus also affect
      // a splice donor or acceptor site, delete a stop codon. Further, the variant might also be a
      // splice region
      ...
      if (changeBeginPos.getPos() % 3 == 0) varTypes.add(VariantEffect.INFRAME_DELETION);
      else varTypes.add(VariantEffect.DISRUPTIVE_INFRAME_DELETION);
      ...
      if (varAAStopPos >= 0) {
        ...
        if (aaChange.getPos() == aaChange.getLastPos()) {
          if (aaChange.getAlt().length() > 0)
            ...
            proteinChange =
                ProteinDeletion.buildWithSequence(
                    true,
                    wtAAFirst,
                    aaChange.getPos(),
                    wtAAFirst,
    ...
    private void handleFrameShiftCase() {
      // The variant is a frameshift deletion. The deletion could span more than one exon and thus
      // also affect a
      // splice donor or acceptor site. Further, the variant might also be stop lost or splice
      // region variant but
      ...
      // A nucleotide deletion can lead to an elongation of the transcript, annotate the specific
      // case.
      ...
      // Normalize the amino acid change, shifting to the right as long as change ref char equals
      // var ref char.
      while (aaChange.getRef().length() > 0
          && aaChange.getPos() < varAASeq.length()
          && aaChange.getRef().charAt(0) == varAASeq.charAt(aaChange.getPos()))
        aaChange = aaChange.shiftRight();

      // Handle the case of deleting a stop codon at the very last entry of the translated amino
      // acid string and
      // short-circuit.
      ...
      if (varAAStopPos >= 0) {
        ...
        if (varTypes.contains(VariantEffect.STOP_LOST))
          proteinChange =
              ProteinExtension.build(true, "*", aaChange.getPos(), varAA, stopCodonOffset);
        else
          proteinChange =
        ...
        if (varTypes.contains(VariantEffect.STOP_LOST))
          proteinChange =
              ProteinExtension.buildWithoutTerminal(true, "*", aaChange.getPos(), varAA);
        else
          proteinChange =
              ProteinFrameshift.buildWithoutTerminal(true, wtAA, aaChange.getPos(), varAA);
      }
    }
  }
}
