...
public class GenomicNucleotideChangeBuilder {
  ...
  public NucleotideChange build() {
    ...
    if (ref.length() == 0)
      ...
      return NucleotideDeletion.buildWithSequence(
          false, beginPos, beginPos + ref.length() - 1, ref);
    else if (ref.length() != alt.length())
      // if tm is null it is intergenic
      return NucleotideIndel.buildWithSequence(
          false, beginPos, beginPos + ref.length() - 1, ref, alt);
    ...
    if (ref.length() == alt.length() && ref.equals(altRC.toString()))
      return NucleotideInversion.buildWithoutSeqDescription(
          false, beginPos, beginPos + ref.length() - 1);
    else
      return NucleotideIndel.buildWithSequence(
          false, beginPos, beginPos + ref.length() - 1, ref, alt);
  }
