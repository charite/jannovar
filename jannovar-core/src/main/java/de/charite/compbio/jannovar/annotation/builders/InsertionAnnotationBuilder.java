...
/**
 * Builds {@link Annotation} objects for the insertion {@link GenomeVariant} in the given {@link
 * TranscriptModel}
 *
 * <h2>Duplications</h2>
 *
 * In the case of insertions that are duplications are annotated as such. These insertions must be
 * in the coding region, such that the duplication can be recognized from the transcript sequence.
 *
 * <h2>Shifting of Insertions</h2>
 *
 * In the case of ambiguities, the HGVS specification requires the variant to be shifted towards the
 * 3' end of the transcript ("rightmost" position). This can cause an insertion to be shifted into
 * the 3' UTR or a splice site.
 *
...
public final class InsertionAnnotationBuilder extends AnnotationBuilder {
   ...
   * @throws InvalidGenomeVariant if <code>change</code> did not describe an insertion
   */
  InsertionAnnotationBuilder(
      TranscriptModel transcript, GenomeVariant change, AnnotationBuilderOptions options)
  ...
  public Annotation build() {
    // Go through top-level cases (clustered by how they are handled here) and build annotations for
    // each of them
    // where applicable.
  ...
  protected NucleotideChange getCDSNTChange() {
    ...
    //
    // The super class constructor will normalize the insertion. Thus, we can work with he
    // assumption that any
    ...
    if (DuplicationChecker.isDuplication(
        transcript.getSequence(), change.getAlt(), txPos.getPos())) {
      ...
      if (change.getAlt().length() == 1) {
        try {
          final NucleotideRange range =
              new NucleotideRange(
                  posBuilder.getNucleotidePointLocation(
                      projector.transcriptToGenomePos(txPos.shifted(-1))),
                  posBuilder.getNucleotidePointLocation(
                      projector.transcriptToGenomePos(txPos.shifted(-1))));
        ...
        try {
          final NucleotidePointLocation firstPos =
              posBuilder.getNucleotidePointLocation(
                  projector.transcriptToGenomePos(txPos.shifted(-change.getAlt().length())));
          final NucleotidePointLocation lastPos =
              posBuilder.getNucleotidePointLocation(
                  projector.transcriptToGenomePos(txPos.shifted(-1)));
   ...
   *
   * <p>We use this helper class to simplify the access to the parameters such as {@link #wtCDSSeq}
   * etc.
   */
  private class CDSExonicAnnotationBuilder {
    ...
    // We keep the following three variables as state of the algorithm since we do not have
    // easy-to-use triples in
    // Java.
    ...
    public CDSExonicAnnotationBuilder() {
      ...
      // Build initial aaChange. This is correct for non-FS insertions, and the first affected bases
      // for FS
      // insertions
    ...
    public Annotation build() {
      // Guard against the case that aaChange describes a "" -> "" change (synonymous change in stop
      // codon).
    ...
    private void handleFrameShiftCase() {
      // Differentiate the cases where the WT has a stop codon at the change position. We also need
      // to guard
    ...
    private boolean handleInsertionAtEndInCaseOfNoStopCodon() {
      // TODO(holtgrew): At some point, try to merge these corner cases that are caused by bogus
      // transcript
      ...
      proteinChange =
          ProteinInsertion.buildWithSequence(
              true,
    ...
    private void handleFrameShiftCaseWTStartWithStopCodon() {
      ...
      if (varAAStopPos == varAAInsertPos) {
        ...
        // The variant peptide contains a stop codon but does not start with it, is frameshift
        // insertion. In
        // this case we cannot really differentiate this from a non-frameshift insertion but we
        // still call
        // it so.
        proteinChange =
            ProteinExtension.build(
                true,
                "*",
                varAAInsertPos,
                toString(varAASeq.charAt(varAAInsertPos)),
                (varAAStopPos - varAAInsertPos));
        varTypes.add(VariantEffect.FRAMESHIFT_ELONGATION);
      } else {
        // The variant AA does not contain a stop codon, is stop loss.
        proteinChange =
            ProteinFrameshift.buildWithoutTerminal(
                true,
                toString(varAASeq.charAt(varAAInsertPos)),
                varAAInsertPos,
                toString(varAASeq.charAt(varAAInsertPos)));
        ...
        if (varAAStopPos == varAAInsertPos) {
          ...
          proteinChange =
              ProteinSubstitution.build(
                  true, toString(wtAASeq.charAt(varAAInsertPos)), varAAInsertPos, "*");
          varTypes.add(VariantEffect.STOP_GAINED);
        } else if (varAAStopPos > varAAInsertPos) {
          // The insertion is a frameshift variant that leads to a transcript still having a stop
          // codon,
          // simple frameshift insertion.
          proteinChange =
              ProteinFrameshift.build(
                  true,
                  toString(wtAASeq.charAt(varAAInsertPos)),
                  varAAInsertPos,
                  toString(varAASeq.charAt(varAAInsertPos)),
      ...
      if (varAAStopPos == 0) {
        ...
        proteinChange =
            ProteinExtension.build(
                ...
                (varAAStopPos - aaChange.getPos()));
        // last is stop codon AA pos
        varTypes.add(VariantEffect.STOP_LOST);
      } else {
        // varAA contains no stop codon
        proteinChange =
            ProteinExtension.buildWithoutTerminal(
                true,
                toString(wtAASeq.charAt(aaChange.getPos())),
                aaChange.getPos(),
                toString(varAASeq.charAt(aaChange.getPos())));
        varTypes.add(VariantEffect.STOP_LOST);
      }
    ...
    private void handleNonFrameShiftCaseStartsWithNoStopCodon() {
      ...
      if (aaChange.getPos() == 0) {
        // The mutation affects the start codon, is start loss (in the case of keeping the start
        // codon
        ...
        if (varAAStopPos == varAAInsertPos) {
          ...
          if (varAAStopPos != -1
            ...
            // The insertion does not directly start with a stop codon but the insertion leads to a
            // stop
            ...
            // The changes on the amino acid level do not lead to a new stop codon, is non-FS
            // insertion.
            ...
            if (aaChange.getRef().equals("")) {
              ...
              if (DuplicationChecker.isDuplication(wtAASeq, aaChange.getAlt(), varAAInsertPos)) {
                ...
                if (aaChange.getAlt().length() == 1) {
                  proteinChange =
                      ProteinDuplication.buildWithSeqDescription(
                          true,
                          toString(wtAASeq.charAt(varAAInsertPos - 1)),
                          varAAInsertPos - 1,
                          toString(wtAASeq.charAt(varAAInsertPos - 1)),
    ...
    /**
     * Add insertion effect to {@link #varTypes}, depending on whether the insertion is disruptive
     * or not.
     */
