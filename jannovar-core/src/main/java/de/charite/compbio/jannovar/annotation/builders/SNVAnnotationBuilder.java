...
/**
 * Builds {@link Annotation} objects for the SNV {@link GenomeVariant}s in the given {@link
 * TranscriptModel}
 *
...
public final class SNVAnnotationBuilder extends AnnotationBuilder {
   ...
   *
   * <p>For changes in coding regions, this is necessary since the transcript might not be the same
   * as the reference (that the VCF file is generated from).
   */
  ...
  private Annotation buildCDSExonicAnnotation() {
    ...
    // Check that the WT nucleotide from the transcript is consistent with change.ref and generate a
    // warning message
    ...
    // Get the transcript codon. From this, we generate the WT and the variant codon. This is
    // important in the case
    // where the transcript differs from the reference. This inconsistency of the reference and the
    // transcript is
    // not necessarily an error in the data base but can also occur in the case of
    ...
    try {
      ...
      return new Annotation(
          ...
          getGenomicNTChange(),
          getCDSNTChange(),
          ProteinMiscChange.build(true, ProteinMiscChangeType.DIFFICULT_TO_PREDICT),
    ...
    // Construct the HGSV annotation parts for the transcript location and nucleotides (note that
    // HGSV uses 1-based
    // positions).
    char wtNT = wtCodon.charAt(frameShift); // wild type nucleotide
    char varNT = varCodon.charAt(frameShift); // wild type amino acid
    ntSubstitutionOverride =
        new NucleotideSubstitution(
            false,
            ntChangeRange.getFirstPos(),
            Character.toString(wtNT),
            Character.toString(varNT));
  ...
  /**
   * @param wtAA wild type amino acid
   * @param varAA variant amino acid
