...
/**
 * Build annotation for {@link SVDuplication}.
 * <p>
 ...
 */
@Immutable
final public class SVDuplicationAnnotationBuilder extends SVAnnotationBuilder {

	/**
	 * The SVDuplication to be annotated.
	 */
	private final SVDuplication svDup;

	/**
	 * Construct the builder.
	 *
	 * @param transcript The transcript to annotate for.
	 * @param svDup      The {@link SVDuplication} to annotate for.
	 */
	public SVDuplicationAnnotationBuilder(TranscriptModel transcript, SVDuplication svDup) {
		super(transcript, svDup);
		this.svDup = svDup;
	}

	@Override
	public SVAnnotation build() {
		final GenomeInterval changeInterval = svDup.getGenomeInterval();
		...
		if (changeInterval.overlapsWith(transcript.getTXRegion())) {
			return new SVAnnotation(svDup, transcript, buildEffectSet(Sets.immutableEnumSet(
