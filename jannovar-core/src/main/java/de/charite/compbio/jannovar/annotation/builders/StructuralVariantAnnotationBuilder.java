...
// TODO(holtgrem): Make AnnotationBuilder an interface and rename AnnotationBuilder to
// AnnotationBuilderBase?

/**
 * Class providing static functions for creating {@link Annotation} objects for SVs.
 *
 * <p>This is currently not inheriting from {@link AnnotationBuilder} since it uses non of its
 * functionality.
 *
...
public final class StructuralVariantAnnotationBuilder {
  ...
  /**
   * Initialize the builder for the structural variant {@link GenomeVariant} in the given {@link
   * TranscriptModel}.
   *
   * @param transcript {@link TranscriptModel} for the transcript to compute the affection for, use
   *     <code>null</code> for intergenic variants
   * @param change {@link GenomeVariant} to compute the annotation for, must describe a structural
   *     variant affecting <code>transcript</code>
   */
  ...
  public Annotation build() {
    ...
    final AnnotationLocation annoLoc =
    ...
    if (ref.length() == alt.length() && ref.equals(altRC.toString())) { // SV inversion
      if (transcript == null) {
        return new Annotation(
            null,
            change,
            ImmutableList.of(VariantEffect.INTERGENIC_VARIANT, VariantEffect.STRUCTURAL_VARIANT),
            null,
            ntChange,
            null,
            null);
        ...
        return new Annotation(
            transcript,
            change,
            ImmutableList.of(VariantEffect.STRUCTURAL_VARIANT),
            annoLoc,
            ntChange,
            null,
            null);
