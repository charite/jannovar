...
public final class StructuralVariantAnnotationBuilder {
	...
	public Annotation build() {
		...
		if (ref.length() == alt.length() && ref.equals(altRC.toString())) { // SV inversion
			if (transcript == null) {
				return new Annotation(null, change, ImmutableList.of(VariantType.INTERGENIC_VARIANT,
						VariantType.STRUCTURAL_VARIANT), null, StringUtil.concatenate("g.", beginPos + 1, "_", beginPos
								+ ref.length(), "inv"), null);
			} else {
				return new Annotation(transcript, change, ImmutableList.of(VariantType.STRUCTURAL_VARIANT), annoLoc,
			...
			if (transcript == null) {
				return new Annotation(null, change, ImmutableList.of(VariantType.INTERGENIC_VARIANT,
						VariantType.STRUCTURAL_VARIANT), null, StringUtil.concatenate("g.", beginPos, "_",
			...
			if (transcript == null) {
				...
				return new Annotation(null, change, ImmutableList.of(VariantType.STRUCTURAL_VARIANT), annoLoc,
