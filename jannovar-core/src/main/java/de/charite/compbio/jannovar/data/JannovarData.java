 ...
 *
 * <p>Making this class immutable makes it a convenient serializeable read-only database.
 *
...
public final class JannovarData implements Serializable {
   ...
   *
   * @param refDict the {@link ReferenceDictionary} to use in this object
   * @param transcriptModels the list of {@link TranscriptModel} objects to use in this object
   */
  public JannovarData(
      ReferenceDictionary refDict, ImmutableList<TranscriptModel> transcriptModels) {
  ...
  /**
   * @param transcriptModels set of {@link TranscriptModel}s to build multi-mapping for
  ...
  /**
   * @param transcriptModels set of {@link TranscriptModel}s to build mapping for
   ...
   * {@link TranscriptModel} objects are entered into an {@link IntervalArray} for the appropriate
   * Chromosome.
   *
   * @param refDict the {@link ReferenceDictionary} to use for the construction
   * @param transcriptModels list of {@link TranscriptModel} objects with the transcripts of all
   *     chromosomes
  ...
  private static ImmutableMap<Integer, Chromosome> makeChromsomes(
      ReferenceDictionary refDict, ImmutableList<TranscriptModel> transcriptModels) {
    ImmutableMap.Builder<Integer, Chromosome> builder =
        new ImmutableMap.Builder<Integer, Chromosome>();
    ...
    for (Integer chrID : transcripts.keySet()) {
      IntervalArray<TranscriptModel> iTree =
          new IntervalArray<TranscriptModel>(
              transcripts.get(chrID), new TranscriptIntervalEndExtractor());
