 ...
 *
 * <p>The query results are sorted lexicographically by <code>(begin, end)</code>.
 *
...
public final class IntervalArray<T> implements Serializable {
  ...
  /**
   * Query the encoded interval tree for all values with intervals overlapping with a given <code>
   * point</code>.
   *
  ...
  /**
   * Implementation of in-order traversal of the encoded tree with pruning using {@link
   * Interval#maxEnd}.
   *
   ...
   * @param result {@link QueryResultBuilder} to add values to
   */
  ...
  public QueryResult findOverlappingWithInterval(int begin, int end) {
    ...
    findOverlappingWithInterval(
        0, intervals.size(), intervals.size() / 2, begin, end, resultBuilder);
  ...
  private class IntervalListBuilder {
    ...
    class TwoIntervalList {
      ...
      public TwoIntervalList(
          ImmutableList<Interval<T>> intervals, ImmutableList<Interval<T>> intervalsEnd) {
    ...
    private int computeMaxEndProperties(
      ...
      mi.setMaxEnd(
          Math.max(
              mi.getMaxEnd(),
              Math.max(
                  computeMaxEndProperties(lst, beginIdx, centerIdx),
