 ...
 *
 * <p>The mapping between chromosome names and RefSeq/GeneBank IDs is done using a RefSeq
 * "chr_accessions_*" file and the chromosome information is retrieved from the UCSC
 * chromInfo.txt.gz file.
 *
...
public final class ReferenceDictParser {
  ...
  public ReferenceDictionary parse() throws TranscriptParseException {
    ...
    //
    // So far, I found two formats. The more recent files at NCBI start with "chr_accessions" and
    // (for human and
    // mouse) contain mappings from chromosomes to other contig names. The older files are called
    // "chr_NC_gi" and
    // contain a mapping from chromosome name to sequence accession number, gi number, potentially
    // assembly unit and
    // then the assembly name. For the latter case, we allow filtering to lines where the last field
    // matches the
    // configuration entry "chrToAccessions.matchLast".
    // TODO(holtgrem): improve documentation for the chrToAccessions.* keys in the
    ...
    if ("chr_accessions".equals(chrToAccessionsFormat)) {
      ...
      for (ImmutableList<String> line : accessionLines) {
        ...
        builder.putContigID(
            "chr" + line.get(CA_CHROMOSOME), chrID); // e.g. "chr1", "chrX", for UCSC
   ...
   */
  private ImmutableList<ImmutableList<String>> loadTSVFile(String path)
      throws TranscriptParseException {
    ImmutableList.Builder<ImmutableList<String>> result =
        new ImmutableList.Builder<ImmutableList<String>>();
    String line;
