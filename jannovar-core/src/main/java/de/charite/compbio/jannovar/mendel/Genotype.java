 ...
 *
 * <p>Genotypes are represented by lists of integers identifying alleles from a {@link
 * GenotypeCalls}. By convention, the reference allele is represented by the integer <code>0</code>.
 * <code>-1</code> encodes no-call.
 *
 * <p>Jannovar will define the zygosity in a very soft way. We do not want to throw too much things
 * away. If all {@link Genotype} are no-calls then teh gebnotype is not observed. But if we have one
 * {@link Genotype} called and another is a no-call (e.g 0/.) we will consider fo the no-call all
 * possibilities. So it can be het or homRef. This behaviour is different to HTSJDK VariantContexed
 * where they have a additional mixed class for such genotypes.
 *
...
public class Genotype {
  ...
  /**
   * @return <code>true</code> if the sample is homozygous ref. Can have exactly one {@value
