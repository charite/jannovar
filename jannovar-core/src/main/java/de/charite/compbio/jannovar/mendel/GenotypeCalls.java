 ...
 *
 * <p>This list contains the core information for the filtration of variants by mendelian
 * inheritance.
 *
 * <p>This list is not called <code>GenotypeList</code> as "list" indicates more of a "vertical"
 * arrangement (multiple sites) of genotypes instead of a "horizontal" one (one site, multiple
 * samples).
 *
 * <p>Note: of course, the class is only immutable as long as <code>payload</code> is immutable!
 *
...
public final class GenotypeCalls implements Iterable<Entry<String, Genotype>> {
  ...
  private static final Genotype GT_NO_CALL = new Genotype(ImmutableList.of(Genotype.NO_CALL));
   ...
   */
  public GenotypeCalls(
      ChromosomeType chromType, Iterable<? extends Entry<String, Genotype>> sampleToGenotype) {
    this(chromType, sampleToGenotype, null);
  }

  /**
   * Initialize {@link GenotypeCalls} with mapping from sample to genotype and an additional
   * "payload" object
   *
   * @param chromType type of the chromosome of this genotype call site
   * @param sampleToGenotype {@link Iterable} with mapping from sample name to {@link Genotype}
   * @param payload An arbitrary payload object. This could be something to later match the
   *     constructed <code>GenotypeCalls</code> back to an object in your application (e.g., the
   *     HTSJDK <code>VariantContext</code> that was used for constructing the {@link
   *     GenotypeCalls}).
   */
  public GenotypeCalls(
      ChromosomeType chromType,
      Iterable<? extends Entry<String, Genotype>> sampleToGenotype,
      Object payload) {
   ...
   * @return {@link Genotype} for the given sample, a not-observed genotype if the sample is unknown
  ...
  public String toString() {
    return "GenotypeCalls [chromType="
        + chromType
        + ", sampleToGenotype="
        + sampleToGenotype
        + ", sampleNames="
