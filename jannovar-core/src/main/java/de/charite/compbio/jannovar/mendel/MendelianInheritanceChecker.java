...
/**
 * Facade class for checking lists of {@link GenotypeCalls} for compatibility with mendelian
 * inheritance
 *
...
public final class MendelianInheritanceChecker {
  ...
  public MendelianInheritanceChecker(Pedigree pedigree) {
    ...
    ImmutableMap.Builder<SubModeOfInheritance, AbstractMendelianChecker> builder =
        new ImmutableMap.Builder<>();
    builder.put(SubModeOfInheritance.AUTOSOMAL_DOMINANT, new MendelianCheckerAD(this));
    builder.put(
        SubModeOfInheritance.AUTOSOMAL_RECESSIVE_COMP_HET, new MendelianCheckerARCompoundHet(this));
   ...
   *
   * @param calls {@link Collection} of {@link GenotypeCalls} objects to perform the mode of
   *     inheritance check for in case of non-recessive mode of inheritance
   * @param recessiveCalls {@link Collection} of {@link GenotypeCalls} objects to perform the mode
   *     of inheritance check for in case of recessive mode of inheritance
   * @return {@link Map} that, for each {@link ModeOfInheritance}, contains the {@link Collection}
   *     of compatible {@link GenotypeCalls} from <code>list</code>
  ...
  public ImmutableMap<ModeOfInheritance, ImmutableList<GenotypeCalls>> checkMendelianInheritance(
      Collection<GenotypeCalls> calls, Collection<GenotypeCalls> recessiveCalls)
      throws IncompatiblePedigreeException {
    ImmutableMap.Builder<ModeOfInheritance, ImmutableList<GenotypeCalls>> builder =
        new ImmutableMap.Builder<>();
    for (ModeOfInheritance mode : ModeOfInheritance.values()) {
      if (mode == ModeOfInheritance.ANY) {
        ...
        if (mode == ModeOfInheritance.AUTOSOMAL_RECESSIVE
            || mode == ModeOfInheritance.X_RECESSIVE) {
   ...
   *
   * @param calls {@link Collection} of {@link GenotypeCalls} objects to perform the mode of
   *     inheritance check for
   * @param compHetRecessiveCalls {@link Collection} of {@link GenotypeCalls} objects to perform the
   *     mode of inheritance check for in case of compound het. recessive mode of inheritance
   * @return {@link Map} that, for each {@link SubModeOfInheritance}, contains the {@link
   *     Collection} of compatible {@link GenotypeCalls} from <code>list</code>
   * @throws IncompatiblePedigreeException if the individuals in <code>calls</code> do not fit to
   *     the pedigree
   */
  public ImmutableMap<SubModeOfInheritance, ImmutableList<GenotypeCalls>>
      checkMendelianInheritanceSub(
          Collection<GenotypeCalls> calls, Collection<GenotypeCalls> compHetRecessiveCalls)
          throws IncompatiblePedigreeException {
    ImmutableMap.Builder<SubModeOfInheritance, ImmutableList<GenotypeCalls>> builder =
        new ImmutableMap.Builder<>();
    for (SubModeOfInheritance mode : SubModeOfInheritance.values()) {
      if (mode == SubModeOfInheritance.ANY) {
        builder.put(mode, ImmutableList.copyOf(calls));
      } else {
        if (mode == SubModeOfInheritance.AUTOSOMAL_RECESSIVE_COMP_HET
            || mode == SubModeOfInheritance.X_RECESSIVE_COMP_HET) {
  ...
  public ImmutableMap<SubModeOfInheritance, ImmutableList<GenotypeCalls>>
      checkMendelianInheritanceSub(Collection<GenotypeCalls> calls)
          throws IncompatiblePedigreeException {
   ...
   *     the pedigree
   */
  public ImmutableList<GenotypeCalls> filterCompatibleRecords(
    ...
    switch (mode) {
      ...
      case AUTOSOMAL_RECESSIVE:
        calls1 =
            ImmutableSet.copyOf(
                checkers
                    .get(SubModeOfInheritance.AUTOSOMAL_RECESSIVE_HOM_ALT)
      ...
      case X_RECESSIVE:
        calls1 =
            ImmutableSet.copyOf(
                checkers
                    .get(SubModeOfInheritance.X_RECESSIVE_HOM_ALT)
                    .filterCompatibleRecords(calls));
        calls2 =
            ImmutableSet.copyOf(
                checkers
   ...
   *     the pedigree
   */
  public ImmutableList<GenotypeCalls> filterCompatibleRecordsSub(
