 ...
 *
 * For autosomal dominant inheritance there must be at least one {@link Genotype} that is shared by
 * all affected individuals but no unaffected individuals in the pedigree. We do not allow
 * homozygous alternative for any affected individuals (and also for the one person in these of
 * singleton pedigrees since this is not the interesting case for users of this class.
 *
...
public class MendelianCheckerAD extends AbstractMendelianChecker {
  ...
  /**
   * @return whether <code>calls</code> is compatible with AD inheritance in the case of a single
  ...
  /**
   * @return whether <code>calls</code> is compatible with AD inheritance in the case of multiple
