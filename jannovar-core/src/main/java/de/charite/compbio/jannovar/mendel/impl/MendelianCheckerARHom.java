 ...
 * {@link Pedigree} and autosomal recessive homozygous mode of inheritance.
 *
 ...
 * unaffected parents of affected individuals are not homozygous ref or homozygous alt and that the
 * unaffected individuals are not homozygous alt. The affected individuals are compatible if no
 * affected individual is homozygous ref or heterozygous and there is at least one affected
 * individual that is homozygous alt.
 *
...
public class MendelianCheckerARHom extends AbstractMendelianChecker {
  ...
  public ImmutableList<GenotypeCalls> filterCompatibleRecords(Collection<GenotypeCalls> calls)
    ...
    Stream<GenotypeCalls> autosomalCalls =
  ...
  /**
   * @return whether <code>calls</code> is compatible with AR homozygous inheritance in the case of
  ...
  private boolean isCompatibleFamily(GenotypeCalls calls) {
    return (affectedsAreCompatible(calls)
        && unaffectedParentsOfAffectedAreNotHomozygous(calls)
