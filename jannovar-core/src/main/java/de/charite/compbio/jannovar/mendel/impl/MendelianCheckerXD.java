 ...
 *
 * For X-chromosomal dominant inheritance, there must be at least one {@link Genotype} that is
 * shared by all affected individuals but no unaffected individuals in the pedigree.
 *
    ...
    for (Person p : pedigree.getMembers()) {
      ...
      if (d == Disease.AFFECTED) {
        if (gt.isHomRef() || (sex == Sex.FEMALE && gt.isHomAlt())) {
          // We do not allow hom. alternative for females to have the same behaviour as AD for
          // females
          return false;
        } else if (sex == Sex.FEMALE && gt.isHet()) {
          numAffectedWithVar++;
        } else if (sex != Sex.FEMALE && (gt.isHet() || gt.isHomAlt())) {
          // We allow heterozygous here as well in the case of mis-calls in the one X copy in the
          // male or
          // unknown
          numAffectedWithVar++;
        }
