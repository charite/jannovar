...
/**
 * Helper class for checking a {@link GenotypeCalls} for compatibility with a {@link Pedigree} and
 * XR homozygous mode
 *
 ...
 * unaffected parents of hom. alt. unaffected females are not are not hom. ref., and that all
 * unaffected individuals are not hom. alt. The affected individuals are compatible if no affected
 * individual is hom. ref. or het. and there is at least one affected individual that is hom. alt.
 *
...
public class MendelianCheckerXRHom extends AbstractMendelianChecker {
  ...
  /**
   * @return whether <code>calls</code> is compatible with XR homozygous inheritance in the case of
  ...
  private boolean isCompatibleFamily(GenotypeCalls calls) {
    return (affectedsAreCompatible(calls)
        ...
        && unaffectedsAreCompatible(calls));
  }
  ...
  /**
   * We only have to look for the parents of the female affected for inherited variants. Here the
   * variant must be inherited from both!
   *
