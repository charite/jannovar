...
/**
 * Wrapper for a immutable lists of {@link Genotype} calls for one {@link TranscriptModel}, one list
 * of calls for each individual.
 *
 * <p>This name list is used for ensuring that the same order and number of individuals is used in
 * the genotype file as in the pedigree file.
 *
...
public final class GenotypeList {
   ...
   * @param isXChromosomal <code>true</code> if the gene list is on the X chromosome, only affects
   *     X-linked compatibility checks (setting this to <code>true</code> can lead to too false
   *     positives and but neither <code>true</code> nor <code>false</code> can lead to false
   *     negatives, given the filter's properties)
  ...
  /**
   * Check whether the {@link #names} of this GenotypeCalls are the same as the names of the members
   * of <code>pedigree</code>.
   *
   * <p>For this, the order of the names has to be the same as the number of the names. This check
