...
public final class Pedigree {
  ...
  /**
   * Initialize the object with the members of <code>contents</code> that have the pedigree name
   * equal to <code>pedigreeName</code>.
   *
   * @param contents contents from the pedigree file
   * @param pedigreeName name of the pedigree to extract
   * @throws PedParseException in the case of problems with references to individuals for mother and
   *     father
   */
   ...
   *
   * <p>If a {@link Person} is selected that has parents in <code>this</code> but the parent's name
   * is not in <code>name</code> then the {@link Person} will have <code>null</code> as the parent
   * object.
   *
