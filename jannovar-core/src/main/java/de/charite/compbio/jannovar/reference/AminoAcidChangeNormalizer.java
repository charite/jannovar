...
public final class AminoAcidChangeNormalizer {
   ...
   *
   * <p>Return <code>change</code> if it is not a clean deletion.
   *
  ...
  public static AminoAcidChange normalizeDeletion(String ref, AminoAcidChange change) {
    ...
    changeRefBuilder.append(
        ref.substring(change.getPos() + shift, change.getPos() + shift + change.getRef().length()));
   ...
   *
   * <p>Truncating of the prefixes is given higher priority to conform with the HGVS notation (you
   * have to call {@link #shiftInsertion}) afterwards.
   *
   ...
   *
   * <p>Returns <code>aaChange</code> if <code>aaChange.ref</code> is not the empty string.
   *
