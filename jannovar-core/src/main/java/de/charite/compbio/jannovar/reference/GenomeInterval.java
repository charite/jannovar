 ...
 *
 * <p>Internally, positions are always stored zero-based, but the position type can be explicitly
 * given to the constructor of {@link GenomeInterval}.
 *
...
public final class GenomeInterval implements Serializable, Comparable<GenomeInterval> {
  ...
  public GenomeInterval(
      ReferenceDictionary refDict, Strand strand, int chr, int beginPos, int endPos) {
  ...
  /**
   * construct genome interval from {@link GenomePosition} with a length towards 3' of pos'
   * coordinate system
   */
  ...
  /**
   * @return a {@link GenomeInterval} that has <code>padding</code> more bases towards each side as
   *     padding
   */
  ...
  /**
   * @return a {@link GenomeInterval} that has <code>padding</code> more bases towards either side
   *     as padding.
   */
  public GenomeInterval withMorePadding(int paddingUpstream, int paddingDownstream) {
    ...
    return new GenomeInterval(
        ...
        strand,
        chr,
        beginPos - paddingUpstream,
        endPos + paddingDownstream,
  ...
  public int compareTo(GenomeInterval other) {
    ...
    return ComparisonChain.start()
        .compare(getChr(), other.getChr())
        .compare(getBeginPos(), other.getBeginPos())
        .compare(getEndPos(), other.getEndPos())
        .result();
  }
}
