 ...
 *
 * <p>Internally, positions are always stored zero-based, but the position type can be explicitely
 * given to the constructor of {@link GenomePosition}.
 *
 * <p>In the case of one-based position, {@link #pos} points to the {@link #pos}-th base in string
 * from the left when starting to count at 1. In the case of zero-based positions, {@link #pos}
 * points to the gap left of the character in the case of positions on the forward strand and to the
 * gap right of the character in the case of positions on the reverse strand. When interpreting this
 * for the reverse strand (i.e. counting from the right), the position right of a character is
 * interpreted as the gap <b>before</b> the character.
 *
 * <p>Reverse-complementing a zero-based GenomePosition must be equivalent to reverse-complementing
 * its one-based position representation. Thus, they are shifted towards the right gap besides the
 * character they point at when changing the strand.
 *
...
public final class GenomePosition implements Serializable, Comparable<GenomePosition> {
  ...
  public GenomePosition(
      ReferenceDictionary refDict, Strand strand, int chr, int pos, PositionType positionType) {
    this.refDict = refDict;
    this.strand = strand;
    this.chr = chr;
  ...
  public GenomePosition(GenomePosition other, Strand strand) {
    ...
    if (strand == other.strand) this.pos = other.pos;
    else this.pos = refDict.getContigIDToLength().get(other.chr) - other.pos - 1;
  }
  ...
  public boolean isLt(GenomePosition other) {
    if (other.strand != strand) other = other.withStrand(strand);
  ...
  /**
   * @param pos other position to compute difference to
   * @return the result of <code>(this.pos - pos.pos)</code> (<code>pos</code> is adjusted to the
   *     coordinate system and strand of <code>this</code>)
   * @throws InvalidCoordinateException if <code>this</code> and <code>pos</code> are on different
   *     chromosomes
   */
  public int differenceTo(GenomePosition pos) {
    if (chr != pos.chr)
      throw new InvalidCoordinateException(
          "Coordinates are on different chromosomes " + this + " vs. " + pos);
    if (pos.strand != strand) pos = pos.withStrand(strand);
    return (this.pos - pos.pos);
  }

  /**
   * @param itv interval to compute difference to
   * @return difference to closest base in <code>itv</code>, as would be returned by {@link
   *     #differenceTo(GenomePosition)}), <code>0</code> if <code>this</code> lies in <code>itv
   *     </code>
   * @throws InvalidCoordinateException if <code>this</code> and <code>itv</code> are on different
   *     chromosomes
   */
  public int differenceTo(GenomeInterval itv) {
    if (chr != itv.getChr())
      throw new InvalidCoordinateException(
          "Coordinates are on different chromosomes " + this + " vs. " + itv);
    if (itv.getStrand() != strand) itv = itv.withStrand(strand);

    if (itv.contains(this)) return 0;

    int a = differenceTo(itv.getGenomeBeginPos());
    if (itv.length() == 0) return a;
   ...
   *
   * <p>The position is shifted towards the 3' end of current strand if <code>delta &gt; 0</code>
   * and towards the 5' end otherwise.
   *
  ...
  public boolean equals(Object obj) {
    ...
    if (strand.isReverse()) return withStrand(Strand.FWD).equals(obj);
