...
public final class GenomeVariantNormalizer {
   ...
   * @param change the genome change for which we want to return the HGVS-normalized representation
   *     for
   * @param txPos the corresponding position on the transcript
   * @return normalized {@link GenomeVariant}
   */
  public static GenomeVariant normalizeGenomeChange(
      TranscriptModel transcript, GenomeVariant change, TranscriptPosition txPos) {
   ...
   * GenomeVariant}.
   *
   * <p>If necessary, the strand of <code>change</code> is set to the same as <code>transcript
   * </code>.
   *
   * @param transcript the transcript with the sequence that should be used
   * @param change the genome change for which we want to return the HGVS-normalized representation
   ...
   * @return normalized {@link GenomeVariant}
   */
  public static GenomeVariant normalizeInsertion(
      TranscriptModel transcript, GenomeVariant change, TranscriptPosition txPos) {
    ...
    // Compute shifted transcript position and transform back to the genome position (we allow
    // shifting over introns
   ...
   *
   * <p>This simply works by shifting the interval to the left as long as the first deleted
   * character equals the character after the deleetion.
   *
   * <p>Note that this function should <b>only</b> be called if the change's deletion interval does
   * not span a splice site.
   *
   * @param transcript the transcript with the sequence that should be used
   * @param change the genome change for which we want to return the HGVS-normalized representation
   *     for
   ...
   * @return normalized {@link GenomeVariant}
   */
  public static GenomeVariant normalizeDeletion(
      TranscriptModel transcript, GenomeVariant change, TranscriptPosition txPos) {
