...
public final class NucleotidePointLocationBuilder {
  ...
  private NucleotidePointLocation getCDNANucleotidePointLocationForExonPos(GenomePosition pos) {
    try {
      ...
      if (getCDSRegion().contains(pos)) {
        ...
        return NucleotidePointLocation.buildDownstreamOfCDS(
            tPos.getPos() - tCDSEndPos.getPos() - 1);
      }
  ...
  private NucleotidePointLocation getCDNANucleotidePointLocationForIntronPos(GenomePosition pos) {
    // Determine which exon is the closest one, ties are broken to the downstream direction as in
    // HGVS,
  ...
  private NucleotidePointLocation getCDNANucleotidePointLocationForUpstreamPos(GenomePosition pos) {
    // The upstream position is simply given as "-$count" where $count is the transcript position of
    // the CDS
   ...
   * @return corresponding {@link NucleotidePointLocation}
   */
  private NucleotidePointLocation getCDNANucleotidePointLocationForDownstreamPos(
      GenomePosition pos) {
    // The downstream position is simply given as "*$count" where $count is the genomic base offset
    // after the CDS
    // region.
