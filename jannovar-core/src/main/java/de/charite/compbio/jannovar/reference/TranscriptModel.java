...
public final class TranscriptModel implements Serializable, Comparable<TranscriptModel> {
  ...
  /**
   * Accession number of the transcript (e.g., the UCSC knownGene id - uc011nca.2). The version
   * number may be included.
   */
  private final String accession;

  /**
   * Gene symbol of the known Gene. Can be null for some genes. Note that in annovar, $name2
   * corresponds to the geneSymbol if available, otherwise the kgID is used.
   */
   ...
   *
   * <p><b>Note</b> that in Jannovar, the CDS region includes the start and stop codon.
   */
  ...
  /**
   * The gene ID, from Ensembl (<code>"ENS[MUS]*G0+([0-9]+)"</code>), Entrez ("<code>ENTREZ([0-9]+)
   * </code> "), RefSeq ("<code>gene([0-9]+)</code>"). <code>null</code> for no available gene ID.
   */
   ...
   *
   * <p>See {@link #getAltGeneIDs()} for more information
   */
  ...
  public TranscriptModel(
      ...
      ImmutableList<GenomeInterval> exonRegions,
      String sequence,
      String geneID,
   ...
   *
   * <p>The alternative identifiers used are the values of {@link AltGeneIDType} converted to
   * strings.
   */
  ...
  /**
   * @return <tt>true</tt> if this is a gene-coding transcript, marked by <tt>cdsRegion</tt> being
   *     empty.
   */
  ...
  public GenomeInterval intronRegion(int i) {
    ...
    return new GenomeInterval(
        exonRegionL.refDict,
        exonRegionL.getStrand(),
        exonRegionL.getChr(),
        exonRegionL.getEndPos(),
        exonRegionR.getBeginPos(),
