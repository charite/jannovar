...
public final class TranscriptProjectionDecorator {
  ...
  public String getCDSTranscript() {
    try {
      TranscriptPosition tBeginPos =
          genomeToTranscriptPos(transcript.getCDSRegion().getGenomeBeginPos());
      TranscriptPosition tEndPos =
          genomeToTranscriptPos(transcript.getCDSRegion().getGenomeEndPos());
  ...
  public String getTranscriptStartingAtCDS() {
    try {
      ...
      return transcript
          .getSequence()
          .substring(tBeginPos.getPos(), transcript.getSequence().length());
    } catch (ProjectionException e) {
   ...
   *
   * @param pos genome position to convert
  ...
  public TranscriptPosition genomeToTranscriptPos(GenomePosition pos) throws ProjectionException {
    ...
    for (GenomeInterval region : transcript.getExonRegions()) {
      if (region.contains(pos)) {
        // Note that we have to use *the region's position* as the base object for the difference
        // operation so
        // the case of the transcript on the reverse strand but position on forward strand is
        // handled correctly.
   ...
   *
   * <p>This is computed from the transcript position and the offset of the CDS start position.
   *
   * @param pos genome position to convert
   * @return position on transcript corresponding to <code>pos</code>
   * @throws ProjectionException if the genome position was not valid
  ...
  public CDSPosition genomeToCDSPos(GenomePosition pos) throws ProjectionException {
    ...
    TranscriptPosition cdsStartPos =
        genomeToTranscriptPos(transcript.getCDSRegion().getGenomeBeginPos());
    return new CDSPosition(
        txPos.getTranscript(), txPos.getPos() - cdsStartPos.getPos(), PositionType.ZERO_BASED);
  }

  /**
   * Coordinate conversion from CDS to transcript position.
   *
   * @param pos the position in the CDS transcript
   ...
   * @param pos the position in the CDS transcript
   * @return the corresponding genome position for pos, will be on the same strand as the transcript
   * @throws ProjectionException on problems with the coordinate transformation (outside of the
   *     transcript)
   */
   ...
   *
   * @param pos the position on the transcript
   * @return the corresponding genome position for pos, will be on the same strand as the transcript
   * @throws ProjectionException on problems with the coordinate transformation (outside of the
   *     transcript)
   */
   ...
   *
   * @param exonID the 0-based exonID in transcript order (reverse order in the case of transcripts
   ...
   *
   * <p>Positions upstream of CDS region are projected to the CDS begin position, downstream of CDS
   * are projected to the CDS end, positions in CDS introns are projected to first position of the
   * next CDS exon.
   *
  ...
  public CDSPosition projectGenomeToCDSPosition(GenomePosition pos) {
    ...
    try {
      ...
      if (transcript.getCDSRegion().isRightOf(pos)) {
        ...
        return projector.genomeToCDSPos(
            transcript.getExonRegions().get(intronNum + 1).getGenomeBeginPos());
      }
    } catch (ProjectionException e) {
   ...
   *
   * <p>Positions upstream of CDS region are projected to the CDS begin position, downstream of CDS
   * are projected to the CDS end.
   *
   * @param interval the {@link GenomeInterval} to project
   * @return the corresponding position in the transcript sequence
   */
  public CDSInterval projectGenomeToCDSInterval(GenomeInterval interval) {
    ...
    final CDSPosition cdsEndPos =
        projectGenomeToCDSPosition(interval.getGenomeEndPos().shifted(-1)).shifted(1);
    return new CDSInterval(
        transcript, cdsBeginPos.getPos(), cdsEndPos.getPos(), PositionType.ZERO_BASED);
  }

  /**
   * Translate {@link GenomePosition} to {@link TranscriptPosition} for {@link #transcript}.
   *
   * <p>Positions upstream of TX region are projected to the TX begin position, downstream of TX are
   * projected to the TX end, positions in introns are projected to first position of the next CDS
   * exon.
   *
   * @param pos the position to translate
   * @return the corresponding position in the transcript sequence
  ...
  public TranscriptPosition projectGenomeToTXPosition(GenomePosition pos) {
    ...
    try {
      ...
      if (transcript.getTXRegion().isRightOf(pos)) {
        ...
        // Deletion begins right of CDS, project to end of CDS.
        return new TranscriptPosition(
            transcript, transcript.transcriptLength(), PositionType.ZERO_BASED);
      } else if (soDecorator.liesInExon(pos)) {
        return projector.genomeToTranscriptPos(pos);
      } else { // lies in intron, project to begin position of next exon
        int intronNum = projector.locateIntron(pos);
        return projector.genomeToTranscriptPos(
            transcript.getExonRegions().get(intronNum + 1).getGenomeBeginPos());
      }
    } catch (ProjectionException e) {
  ...
  public TranscriptInterval projectGenomeToTXInterval(GenomeInterval interval) {
    ...
    final TranscriptPosition txEndPos =
        projectGenomeToTXPosition(interval.getGenomeEndPos().shifted(-1)).shifted(1);
    return new TranscriptInterval(
        transcript, txBeginPos.getPos(), txEndPos.getPos(), PositionType.ZERO_BASED);
  }
}
