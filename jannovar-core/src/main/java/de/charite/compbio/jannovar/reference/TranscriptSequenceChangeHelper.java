...
public final class TranscriptSequenceChangeHelper {
  ...
  private String getTranscriptWithRangeInRefAffected(GenomeVariant change) {
    ...
    try {
      ...
      throw new Error(
          "Bug: should be able to translate change begin position to transcript position.");
    }
    ...
    try {
      ...
      throw new Error(
          "Bug: should be able to translate change end position to transcript position.");
    }
   ...
   *
   * <p>Positions upstream of transcript region (TX) are projected to TX begin, positions downstream
   * of TX are projected to TX end, positions in introns are projected to first position of the next
   * TX exon.
   *
  ...
  private TranscriptPosition translateGenomeToTranscriptPosition(GenomePosition pos)
    ...
    if (transcript.getTXRegion().isRightOf(pos)) {
      ...
      return projector.genomeToTranscriptPos(
          transcript.getExonRegions().get(intronNum).getGenomeBeginPos());
    }
  }
   ...
   *
   * <p>We <b>extend</b> the CDS transcript to the right so that the changed protein sequence can be
   * computed for frameshift changes.
   *
