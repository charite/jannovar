...
/**
 * Encapsulate a single variant genotype call (i.e., one line in a VCF file) for an arbitrary number of individuals.
 *
 * The individual calls for each sample are stored in {@link #callList}, and the corresponding qualities are stored in
 * {@link #qualityList}.
 *
 * The class also stores the values for <tt>DP</tt> (read depth at this position for this sample) and <tt>GQ</tt>
 * (genotype quality, encoded as a phred quality <code>-10 * log10(p(genotype call is wrong))</code>, both stored as
 * lists of {@link Integer} values. These fields are present in all VCF files that we are interested in. If there is no
 * value for it, we store <code>0</code>.
 *
 * @author Peter N Robinson <peter.robinson@charite.de>
 */
public class GenotypeCall {

	/** List of {@link Genotype} calls for each variant. */
	private ArrayList<Genotype> callList = null;
	/** List of Phred-scaled qualities for each genotype call in {@link #callList}. */
	private ArrayList<Integer> qualityList = null;
	/** List of read depth values for the genotype calls at hand (VCF <tt>DP</tt> field). */
	private ArrayList<Integer> depthList = null;

	/**
	 * The constructor takes lists of calls and qualities that have been parsed from a single VCF line by the
	 * {@link jannovar.genotype.MultipleGenotypeFactory MultipleGenotypeFactory}. By assumption, there are multiple
	 * samples, which are described elsewhere by a PED file.
	 *
	 * @param calls
	 *            A list of the genotype calls, one for each sample
	 * @param qualities
	 *            A list of the genotype Phred qualities, one for each sample.
	 */
	 ...
	 *            A list of the genotype Phred qualities, one for each sample.
	 * @param depths
	 *            A list of the genotype read depth values, one for each sample.
	 */
	...
	/**
	 * This constructor is intended to be used for VCF files with a single sample, which by assumption contains data
	 * from a patient.
	 *
	 * @param gt
	 *            the {@link Genotype}
	...
	/**
	 * This constructor is intended to be used for VCF files with a single sample, which by assumption contains data
	 * from a patient. This constructor is used to register data about the read depth of the call.
	 *
	 * @param gt
	 *            the {@link Genotype}
	 * @param qual
	...
	/**
	 * Note that this function expects the parameter n to be zero-based. If the n is invalid, return null. This method
	 * is intended mainly for debugging. May need to add exceptions.
	 *
	 * @param n
	 *            entry number
	...
	/**
	 * @param n
	 *            The number of the sample in the VCF file.
	...
	/**
	 * @param n
	 *            Number of individual in pedigree.
	...
	/**
	 * This function returns true if the individual concerned has a heterozygous or homozyous ALT (variant) call for the
	 * current position.
	 *
	 * @param n
	 *            Number of individual in pedigree.
	...
	/**
	 * @param n
	 *            Number of the individual in the VCF file
	...
	/**
	 * This method gets the number of individuals included in the genotype call. This must be equal to the number of
