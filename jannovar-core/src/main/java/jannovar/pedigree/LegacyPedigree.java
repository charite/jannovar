...
public class LegacyPedigree {
	 ...
	 */
	private ArrayList<LegacyPerson> parentList = null;
	/**
	 ...
	 * .
	 */
	private ArrayList<LegacyPerson> unaffectedList = null;
	 ...
	 *
	 * @param name
	 * @return new {@link LegacyPedigree} with one male individual, the given name and in the family <tt>"FAMILY"</tt>.
	 */
	public static LegacyPedigree constructSingleSamplePedigree(String name) {
		ArrayList<LegacyPerson> pList = new ArrayList<LegacyPerson>();
		try {
			pList.add(new LegacyPerson("FAMILY", name, null, null, "0", "2"));
			return new LegacyPedigree(pList, "FAMILY");
	...
	/**
	 * @param pList
	 *            list of {@link LegacyPerson}s
	 * @param famID
	 *            the Family ID of this Pedigree.
	 * @throws jannovar.pedigree.PedParseException
	 */
	public LegacyPedigree(ArrayList<LegacyPerson> pList, String famID) throws PedParseException {
		this.personList = new ArrayList<LegacyPerson>();
		this.familyID = famID;
		for (LegacyPerson p : pList) {
			addIndividual(p);
		}
	...
	public boolean isNthPersonAffected(int n) {
		if (n < 0 || n >= personList.size())
			return false;
		LegacyPerson p = this.personList.get(n);
		return p.isAffected();
	}

	/**
	 * Returns the PED file data for the nth person in the pedigree. See
	 * {@link jannovar.pedigree.LegacyPerson#getPEDFileData getPEDFileData}.
	 *
	 * @param n
	 *            number of the person
	...
	public boolean sampleIsRepresentedInPedigree(String name) {
		for (LegacyPerson p : this.personList) {
	...
	public void adjustSampleOrderInPedFile(ArrayList<String> sampleNames) throws PedParseException {
		 ...
		 */
		ArrayList<LegacyPerson> newList = new ArrayList<LegacyPerson>();
		for (String s : sampleNames) {
			Iterator<LegacyPerson> it = this.personList.iterator();
			while (it.hasNext()) {
				LegacyPerson p = it.next();
	 ...
	 * @throws jannovar.pedigree.PedParseException
	 */
	public void addIndividual(LegacyPerson person) throws PedParseException {
	...
	private void setPersonIndices() {
		for (int i = 0; i < this.personList.size(); ++i) {
			LegacyPerson p = this.personList.get(i);
			p.setIndex(i);
		}
	}
	 ...
	 * each {@link jannovar.pedigree.LegacyPerson Person} object with a known Disease status into one of these three
	 * categories. Persons with an unknown Disease status are not included.
	 */
	private void initializeAffectedsParentsSibs() {
		this.affectedList = new ArrayList<LegacyPerson>();
		this.parentList = new ArrayList<LegacyPerson>();
		this.unaffectedList = new ArrayList<LegacyPerson>();
		 ...
		 */
		for (LegacyPerson p : affectedList) {
			LegacyPerson father = p.getFather();
			LegacyPerson mother = p.getMother();
	...
	private boolean findParentLinks() throws PedParseException {
		HashMap<String, LegacyPerson> personMap = new HashMap<String, LegacyPerson>();
		for (LegacyPerson p : personList) {
			String id = p.getIndividualID();
			personMap.put(id, p);
		}
		for (LegacyPerson p : personList) {
			...
			if (fatherID != null) {
				LegacyPerson father = personMap.get(fatherID);
				if (father == null) {
			...
			if (motherID != null) {
				LegacyPerson mother = personMap.get(motherID);
				if (mother == null) {
	...
	public LegacyPerson get(int idx) {
		return this.personList.get(idx);
	}

	public LegacyPerson getPerson(String id) {
		for (LegacyPerson p : this.personList) {
	 ...
	 * {@link jannovar.pedigree.GenotypeList GenotypeCall} object passed to this function is expected to represent all
	 ...
	 * unaffected persons in the pedigree. The samples represented by the {@link jannovar.pedigree.GenotypeList
	...
	public boolean isCompatibleWithAutosomalDominant(ArrayList<Variant> varList) {
		...
		for (Variant v : varList) {
			GenotypeList multiGT = v.getGenotype();
	 ...
	 */
	private boolean affectedsAreHomozygousALT(GenotypeList multiGT) {
		for (LegacyPerson p : this.affectedList) {
			int idx = p.getIndex();
	 ...
	 */
	private boolean parentsAreHeterozygous(GenotypeList multiGT) {
		for (LegacyPerson p : this.parentList) {
			int idx = p.getIndex();
			Genotype gt = multiGT.getGenotypeInIndividualN(idx);
	 ...
	 */
	private boolean unaffectedsAreNotHomozygousALT(GenotypeList multiGT) {
		for (LegacyPerson p : this.unaffectedList) {
			int idx = p.getIndex();
			Genotype gt = multiGT.getGenotypeInIndividualN(idx);
			if (gt == Genotype.HOMOZYGOUS_ALT)
				return false;
	 ...
	 */
	private boolean affectedsAreHeterozygous(GenotypeList multiGT) {
		for (LegacyPerson p : this.affectedList) {
			int idx = p.getIndex();
			Genotype gt = multiGT.getGenotypeInIndividualN(idx);
			if (gt != Genotype.HETEROZYGOUS)
	 ...
	 */
	private boolean onlyOneParentIsHeterozygous(GenotypeList multiGT) {
		int n = 0;
		for (LegacyPerson p : this.parentList) {
			int idx = p.getIndex();
			Genotype gt = multiGT.getGenotypeInIndividualN(idx);
	...
	private boolean fatherIsHeterozygous(GenotypeList multiGT) {
		for (LegacyPerson p : this.parentList) {
			int idx = p.getIndex();
			Genotype gt = multiGT.getGenotypeInIndividualN(idx);
	...
	private boolean motherIsHeterozygous(GenotypeList multiGT) {
		for (LegacyPerson p : this.parentList) {
			int idx = p.getIndex();
			Genotype gt = multiGT.getGenotypeInIndividualN(idx);
	 ...
	 */
	public boolean containsCompatibleHomozygousVariant(GenotypeList GT) {
		if (this.isSingleSample) {
	 ...
	 */
	private boolean validCompoundHet(GenotypeList matGT, GenotypeList patGT) {
		for (LegacyPerson p : this.unaffectedList) {
			int i = p.getIndex();
	...
	public boolean isCompatibleWithAutosomalRecessiveCompoundHet(ArrayList<Variant> varList) {
		...
		ArrayList<GenotypeList> paternal = new ArrayList<GenotypeList>();
		ArrayList<GenotypeList> maternal = new ArrayList<GenotypeList>();
		 ...
		 */
		for (GenotypeList patGT : paternal) {
			for (GenotypeList matGT : maternal) {
	...
	public boolean isCompatibleWithXChromosomalRecessive(ArrayList<Variant> varList) {
		...
		for (Variant v : varList) {
			...
			for (LegacyPerson p : unaffectedList) {
				int i = p.getIndex();
				Genotype g = gc.getGenotypeInIndividualN(i);
	 ...
	 *
	 * @return summary of this {@link LegacyPedigree}
	 */
