...
public class GenomicNucleotideChangeBuilderTest {
  ...
  public void testInsertion() {
    Assert.assertEquals(
        "100_101insCGAT", new GenomicNucleotideChangeBuilder(varIns).build().toHGVSString());
  }

  @Test
  public void testDeletion() {
    Assert.assertEquals(
        "101_104delCGAT", new GenomicNucleotideChangeBuilder(varDel).build().toHGVSString());
  }

  @Test
  public void testSNV() {
    Assert.assertEquals(
        "101C>T", new GenomicNucleotideChangeBuilder(varSNV).build().toHGVSString());
  }

  @Test
  public void testSubstitution() {
    Assert.assertEquals(
        "101_103delCGAinsTTT", new GenomicNucleotideChangeBuilder(varSub).build().toHGVSString());
  }

  @Test
  public void testInversion() {
    Assert.assertEquals(
        "101_104inv", new GenomicNucleotideChangeBuilder(varInv).build().toHGVSString());
  }
}
