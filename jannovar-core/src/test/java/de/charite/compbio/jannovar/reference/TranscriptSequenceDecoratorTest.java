...
public class TranscriptSequenceDecoratorTest {
  ...
  public void testGetCodonAt() throws Exception {
    ...
    Assert.assertEquals(
        ...
        decorator.getCodonAt(tx(START_LAST_CODON), cds(START_LAST_CODON)));
  ...
  public void testGetCodonsStartingFrom() throws Exception {
    ...
    Assert.assertEquals(
        CODONS[CODONS.length - 2] + CODONS[CODONS.length - 1],
  ...
  private static TranscriptModel getTestTranscriptModel() {
    ...
    return new TranscriptModel(
        "TEST",
        "TEST",
        new GenomeInterval(null, Strand.FWD, 1, 0, sequence.length()),
        new GenomeInterval(null, Strand.FWD, 1, 0, sequence.length()),
