 ...
 *
 * <p>Useful for obtaining reliable de novo variant calls.
 *
 * <p>Note that this filter has to be applied <b>after</b> {@link GenotypeThresholdFilterAnnotator}
...
public class PedigreeFilterAnnotator {
  ...
  private List<Genotype> annotateGenotypes(VariantContextBuilder builder, VariantContext vc) {
    ...
    for (Genotype gt : vc.getGenotypes()) {
      ...
      if (deNovoAllele != null) {
        ...
        final int maxCountInParent = getMaxCountInParents(vc, gt.getSampleName(), deNovoAllele);
    ...
    for (Genotype gt : vc.getGenotypes()) {
      ...
      final Allele deNovoAllele = getDeNovoAllele(vc, gt.getSampleName());
      gtBuilder.attribute(
          PedigreeFilterHeaderExtender.FORMAT_GT_DE_NOVO, (deNovoAllele != null) ? "Y" : "N");
   ...
   */
  private int filteredParentGtCount(
      VariantContext vc, Map<String, List<String>> extraFts, String sampleName) {
    final Person person = this.pedigree.getNameToMember().get(sampleName).getPerson();
    ...
    if (person.getFather() != null) {
      ...
      valFather =
          (gtFather != null && (gtFather.isFiltered() || !extraFts.get(fatherName).isEmpty()))
              ? 1
              : 0;
    } else {
      valFather = 0;
    ...
    if (person.getMother() != null) {
      ...
      valMother =
          (gtMother != null && (gtMother.isFiltered() || !extraFts.get(motherName).isEmpty()))
              ? 1
              : 0;
    } else {
      valMother = 0;
  ...
  private boolean deNovoGtSharedWithSibling(
      VariantContext vc, String sampleName, Allele deNovoAllele) {
    ...
    final ImmutableMap<Person, ImmutableList<Person>> siblings = pedigreeDecorator.buildSiblings();
  ...
  private Allele getDeNovoAllele(VariantContext vc, String sampleName) {
    ...
    if (gtPerson == null
        ...
        || gtFather.isNoCall()
        || gtMother == null
        || gtMother.isNoCall()) return null; // cannot make any judgement
