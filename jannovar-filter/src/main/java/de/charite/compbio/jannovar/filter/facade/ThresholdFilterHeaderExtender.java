  ...
  public void addHeaders(VCFHeader header) {
    ...
    header.addMetaDataLine(
        new VCFFilterHeaderLine(
            FILTER_GT_MAX_COV, "Genotype has coverage >" + options.getMaxCov()));
    header.addMetaDataLine(
        new VCFFilterHeaderLine(
            FILTER_GT_MIN_COV_HET, "Het. genotype call has coverage <" + options.getMinGtCovHet()));
    header.addMetaDataLine(
        new VCFFilterHeaderLine(
            FILTER_GT_MIN_COV_HOM_ALT,
            "Hom. alt genotype call has coverage <" + options.getMinGtCovHomAlt()));
    header.addMetaDataLine(
        new VCFFilterHeaderLine(
            FILTER_GT_MIN_GQ, "Genotype has quality (GQ) <" + options.getMinGtGq()));
    header.addMetaDataLine(
        new VCFFilterHeaderLine(
            FILTER_GT_MIN_AAF_HET,
            "Het. genotype has alternative allele fraction <" + options.getMinGtAafHet()));
    header.addMetaDataLine(
        new VCFFilterHeaderLine(
            FILTER_GT_MAX_AAF_HET,
    ...
    header.addMetaDataLine(
        new VCFFilterHeaderLine(
            FILTER_VAR_MAX_HOM_EXAC,
            "Seen >" + options.getMaxExacHomState() + " times in ExAC data in homozygous state"));

    header.addMetaDataLine(
        new VCFFilterHeaderLine(
            FILTER_VAR_MAX_HOM_THOUSAND_GENOMES,
            "Seen >"
                + options.getMaxG1kHomState()
