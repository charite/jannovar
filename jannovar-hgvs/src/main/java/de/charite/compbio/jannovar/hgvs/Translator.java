...
public final class Translator {
  ...
  /**
   * Map of genetic code. Keys are codons and values are the corresponding amino acid (one-letter
   * code)
   */
  private ImmutableMap<String, String> codon1 = null;
  /**
   * Map of genetic code. Keys are codons and values are the corresponding amino acid (three-letter
   * code)
   */
  ...
  /**
   * Private constructor, initializes singleton instance. Use {@link #getTranslator} for obtaining
   * an object.
   */
  private Translator() {
    initializeMaps();
   ...
   *
   * <p>Currently, there is no need to translate more than a single codon. However, some portions of
   * the code are trying to translate DNA that is not a multiple of 3 nt long (from indel code).
   * Therefore, we will translate as much as possible here. This may need refactoring in the future.
   * (TODO).
   *
  ...
  private String translateDNA(String dnaseq, ImmutableMap<String, String> codonTable) {
    ...
    if (!(len % 3 == 0)) {
      ...
      // String err =
      // String.format("Attempt to translate sequence [%s] with length %d (should be
      // 3n)",dnaseq,len);
  ...
  /**
   * Initializes a set of maps that represent the gene code with various aminoacid codes. Also
   * initializes map of IUPAC codes.
   */
