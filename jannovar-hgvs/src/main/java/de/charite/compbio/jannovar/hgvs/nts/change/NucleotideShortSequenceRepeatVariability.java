...
public class NucleotideShortSequenceRepeatVariability extends NucleotideChange {
  ...
  /**
   * Factory forwards to {@link #NucleotideShortSequenceRepeatVariability(boolean, NucleotideRange,
   * int, int)}.
   */
  public static NucleotideShortSequenceRepeatVariability build(
      boolean onlyPredicted, NucleotideRange range, int minCount, int maxCount) {
    return new NucleotideShortSequenceRepeatVariability(onlyPredicted, range, minCount, maxCount);
  }

  /** Construct with the given values */
  public NucleotideShortSequenceRepeatVariability(
      boolean onlyPredicted, NucleotideRange range, int minCount, int maxCount) {
    super(onlyPredicted);
    this.range = range;
  ...
  public String toHGVSString() {
    return wrapIfOnlyPredicted(
        Joiner.on("").join(range.toHGVSString(), "(", minCount, "_", maxCount, ")"));
  }

  @Override
  public String toString() {
    return "NucleotideShortSequenceRepeatVariability [range="
        + range
        + ", minCount="
        + minCount
        + ", maxCount="
