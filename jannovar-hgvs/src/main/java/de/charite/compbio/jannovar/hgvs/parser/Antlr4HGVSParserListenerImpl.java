...
class Antlr4HGVSParserListenerImpl extends Antlr4HGVSParserBaseListener {
   ...
   *
   * <p>The HGVSVariant for the result is taken from the child and stored in the object member.
   */
  @Override
   ...
   *
   * <p>The result from the child is propagated to the label of this node.
   */
  @Override
   ...
   *
   * <p>Collect sequence type, sequence ID, and NucleotideChange from the children and label ctx
   * with the resulting {@link SingleAlleleNucleotideVariant}.
   */
  @Override
  public void exitNt_single_allele_single_change_var(
    ...
    setValue(
        ctx,
        new SingleAlleleNucleotideVariant(
            seqType,
            refLabel.getTranscriptID(),
            refLabel.getProteinID(),
            refLabel.getTranscriptVersion(),
   ...
   *
   * <p>Construct new {@link SingleAlleleNucleotideVariant} as a label for this node, using {@link
   * NucleotideChangeAllele} from child label.
   */
  @Override
  public void exitNt_single_allele_multi_change_var(Nt_single_allele_multi_change_varContext ctx) {
    ...
    final SequenceType seqType =
        SequenceType.findMatchingForPrefix(ctx.NT_CHANGE_DESCRIPTION().getText());
    final ReferenceLabel refLabel = (ReferenceLabel) getValue(ctx.reference());
    final NucleotideChangeAllele allele =
        (NucleotideChangeAllele) getValue(ctx.nt_multi_change_allele());
    setValue(
        ctx,
        new SingleAlleleNucleotideVariant(
            seqType,
   ...
   *
   * <p>Construct {@link MultiAlleleNucleotideVariant} and set as this node's label from children's
  ...
  public void exitNt_multi_allele_var(Nt_multi_allele_varContext ctx) {
    ...
    setValue(
        ctx,
        new MultiAlleleNucleotideVariant(
            seqType,
            refLabel.getTranscriptID(),
            refLabel.getProteinID(),
            refLabel.getTranscriptVersion(),
   ...
   *
   * <p>Label this node with the label of the child nt_multi_change_allele_inner.
   */
  @Override
  public void exitNt_multi_change_allele(Nt_multi_change_alleleContext ctx) {
    ...
    if (ctx.NT_PAREN_OPEN() == null) {
      ...
      NucleotideChangeAllele allele =
          (NucleotideChangeAllele) getValue(ctx.nt_multi_change_allele_inner());
   ...
   *
   * <p>Construct NucleotideChangeAllele from the children's labels.
   */
  @Override
   ...
   *
   * <p>Construct {@link NucleotideIndel} from children's values and labels and label ctx with this.
   */
  @Override
  public void exitNt_change_indel(Nt_change_indelContext ctx) {
    ...
    if (hasAny && !hasBoth) {
      final ParserRuleContext singleton =
          ctx.nt_number(0) != null ? ctx.nt_number(0) : ctx.nt_string(0);
   ...
   *
   * <p>Construct {@link NucleotideDeletion} from children's values and labels and label ctx with
   * this.
   */
  @Override
   ...
   *
   * <p>Construct {@link NucleotideDuplication} from children's values and labels and label ctx with
   * this.
   */
  @Override
   ...
   *
   * <p>Construct {@link NucleotideInsertion} from children's values and labels and label ctx with
   * this.
   */
  @Override
   ...
   *
   * <p>Construct {@link NucleotideInversion} from children's values and labels and label ctx with
   * this.
   */
  @Override
   ...
   *
   * <p>Construct {@link NucleotideSubstitution} from children's values and labels and label ctx
   * with this.
   */
  @Override
  public void exitNt_change_substitution(Nt_change_substitutionContext ctx) {
    ...
    setValue(
        ctx,
        new NucleotideSubstitution(
            false, position, ctx.NT_STRING(0).getText(), ctx.NT_STRING(1).getText()));
  }

  /**
   * Leaving of nt_change_ssr rule
   *
   * <p>Construct {@link NucleotideShortSequenceRepeatVariability} from the children's values and
   * label ctx with this.
   */
  @Override
   ...
   *
   * <p>Construct {@link NucleotideMiscChange} from the children's values and label ctx with this.
   */
  @Override
   ...
   *
   * <p>Label node with {@link ReferenceLabel}
   */
  @Override
   ...
   *
   * <p>Construction {@link NucleotidePointLocation} from the children's values and label ctx with
   * this.
   */
  @Override
  public void exitNt_point_location(Nt_point_locationContext ctx) {
    ...
    if (ctx.nt_offset() == null) {
      ...
      setValue(
          ctx,
          new NucleotidePointLocation(baseLoc.getBasePos(), offset, baseLoc.isDownstreamOfCDS()));
    }
  }
   ...
   *
   * <p>The LegacyVariant for the result is taken from the child and stored in the object member.
   */
  @Override
   ...
   *
   * <p>Propagate result of child to label of this node.
   */
  @Override
   ...
   *
   * <p>Construct {@link LegacyDeletion} from children's value and label this node with it.
   */
  @Override
   ...
   *
   * <p>Construct {@link LegacySubstitution} from children's value and label this node with it.
   */
  @Override
   ...
   *
   * <p>Construct {@link LegacyIndel} from children's value and label this node with it.
   */
  @Override
   ...
   *
   * <p>Construct {@link LegacyInsertion} from children's value and label this node with it.
   */
  @Override
