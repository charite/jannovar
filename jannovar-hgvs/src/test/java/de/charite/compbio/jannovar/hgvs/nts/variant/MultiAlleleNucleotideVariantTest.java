...
public class MultiAlleleNucleotideVariantTest {
  ...
  public void setUp() throws Exception {
    ...
    singleAlleleVariant =
        new MultiAlleleNucleotideVariant(
            SequenceType.CODING_DNA, "REF", ImmutableList.of(firstAllele));
    multiAlleleVariant =
        new MultiAlleleNucleotideVariant(
            SequenceType.CODING_DNA, "REF", ImmutableList.of(firstAllele, secondAllele));
  }

  public void testStaticFactoryMakeSingleChangeVariant() {
    Assert.assertEquals(
        singleAlleleVariant,
        SingleAlleleNucleotideVariant.makeSingleChangeVariant(
  ...
  public void testStaticFactoryBuild() {
    Assert.assertEquals(
        multiAlleleVariant,
        SingleAlleleNucleotideVariant.build(
            SequenceType.CODING_DNA,
            "REF",
            VariantConfiguration.IN_CIS,
  ...
  public void testToHGVSStringSingleChange() {
    Assert.assertEquals(
        "REF:c.[100A>G;301A>C]", singleAlleleVariant.toHGVSString(AminoAcidCode.ONE_LETTER));
    Assert.assertEquals(
        "REF:c.[100A>G;301A>C]", singleAlleleVariant.toHGVSString(AminoAcidCode.THREE_LETTER));
  ...
  public void testToHGVSStringMultiChange() {
    Assert.assertEquals(
        "REF:c.[100A>G;301A>C];[34T>C;301A>G]",
