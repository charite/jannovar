...
public class NucleotideChangeAlleleTest {
  ...
  public void testSingletonStaticFactory() {
    Assert.assertEquals(
        singletonAllele,
        NucleotideChangeAllele.singleChangeAllele(
            NucleotideSubstitution.build(false, 100, "C", "A")));
  }
  ...
  public void testMultVariantToHGVSString() {
    ...
    Assert.assertEquals(
        "[101C>A//100C>A]", multiChangeAllele.toHGVSString(AminoAcidCode.THREE_LETTER));
    Assert.assertEquals(
        "[101C>A//100C>A]", multiChangeAllele.toHGVSString(AminoAcidCode.ONE_LETTER));
  }
}
