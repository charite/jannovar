...
public class HGVSParserNucleotidePointLocationTest extends HGVSParserTestBase {
  ...
  public void testPositiveNumberAsPointLocation() {
    ...
    Assert.assertEquals(
        "(nt_point_location (nt_base_location (nt_number 123)))", loc.toStringTree(parser));
  }

  @Test
  public void testUpstreamNumberAsPointLocation() {
    Antlr4HGVSParser parser =
        buildParserForString("-123", Antlr4HGVSLexer.NUCLEOTIDE_CHANGE, false);
    Nt_point_locationContext loc = parser.nt_point_location();
    Assert.assertEquals(
        "(nt_point_location (nt_base_location - (nt_number 123)))", loc.toStringTree(parser));
  }
  ...
  public void testPositiveNumberWithPositiveOffsetAsPointLocation() {
    Antlr4HGVSParser parser =
        buildParserForString("123+4", Antlr4HGVSLexer.NUCLEOTIDE_CHANGE, false);
    Nt_point_locationContext loc = parser.nt_point_location();
    Assert.assertEquals(
        "(nt_point_location (nt_base_location (nt_number 123)) (nt_offset + (nt_number 4)))",
