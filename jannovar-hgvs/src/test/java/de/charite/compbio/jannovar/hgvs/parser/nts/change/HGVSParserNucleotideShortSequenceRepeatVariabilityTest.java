...
public class HGVSParserNucleotideShortSequenceRepeatVariabilityTest extends HGVSParserTestBase {
  ...
  public void testLengthOne() {
    Antlr4HGVSParser parser =
        buildParserForString("123(3_4)", Antlr4HGVSLexer.NUCLEOTIDE_CHANGE, false);
    Nt_change_ssrContext nt_change_ssr = parser.nt_change_ssr();
    Assert.assertEquals(
        "(nt_change_ssr (nt_point_location (nt_base_location (nt_number 123))) ( 3 _ 4 ))",
  ...
  public void testLengthTwo() {
    Antlr4HGVSParser parser =
        buildParserForString("123_124(3_4)", Antlr4HGVSLexer.NUCLEOTIDE_CHANGE, false);
    Nt_change_ssrContext nt_change_ssr = parser.nt_change_ssr();
    Assert.assertEquals(
