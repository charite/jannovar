...
public class HGVSparserNucleotideSingleAlleleVariantTest extends HGVSParserTestBase {
	...
	public void testWithOneChangeJustTranscript() {
		...
		Assert.assertEquals(
				"(hgvs_variant (nt_single_allele_var (nt_single_allele_single_change_var (reference NM_000109.3 :) c. (nt_change (nt_change_substitution (nt_point_location (nt_base_location (nt_number 123))) A > C)))))",
	...
	public void testWithOneChangeWithProteinIsoform() {
		...
		Hgvs_variantContext hgvs_variant = parser.hgvs_variant();
		Assert.assertEquals(
				"(hgvs_variant (nt_single_allele_var (nt_single_allele_single_change_var (reference NM_000109.3 ( DMD_i2 ) :) c. (nt_change (nt_change_substitution (nt_point_location (nt_base_location (nt_number 123))) A > C)))))",
				hgvs_variant.toStringTree(parser));
	}
	...
	public void testWithOneChangeWithProteinVersion() {
		...
		Hgvs_variantContext hgvs_variant = parser.hgvs_variant();
		Assert.assertEquals(
				"(hgvs_variant (nt_single_allele_var (nt_single_allele_single_change_var (reference NM_000109.3 ( DMD_v2 ) :) c. (nt_change (nt_change_substitution (nt_point_location (nt_base_location (nt_number 123))) A > C)))))",
				hgvs_variant.toStringTree(parser));
	}
	...
	public void testWithMultiplesChangesSingleOrigin() {
		...
		Hgvs_variantContext hgvs_variant = parser.hgvs_variant();
		Assert.assertEquals(
				"(hgvs_variant (nt_single_allele_var (nt_single_allele_multi_change_var (reference NM_000109.3 :) c. (nt_multi_change_allele [ (nt_multi_change_allele_inner (nt_change (nt_change_substitution (nt_point_location (nt_base_location (nt_number 123))) A > C)) (nt_var_sep ,) (nt_change (nt_change_substitution (nt_point_location (nt_base_location (nt_number 124))) C > T))) ]))))",
				hgvs_variant.toStringTree(parser));
	}
