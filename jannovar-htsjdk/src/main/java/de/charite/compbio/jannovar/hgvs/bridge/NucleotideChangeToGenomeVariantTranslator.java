...
public class NucleotideChangeToGenomeVariantTranslator {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(NucleotideChangeToGenomeVariantTranslator.class);
  ...
  public NucleotideChangeToGenomeVariantTranslator(
      JannovarData jvDB, IndexedFastaSequenceFile indexedFasta) {
    this.jvDB = jvDB;
    this.seqExtractor = new GenomeRegionSequenceExtractor(jvDB, indexedFasta);
  }

  /**
   * Shortcut to {@link #translateNucleotideVariantToGenomeVariant(SingleAlleleNucleotideVariant,
   * boolean)} with using <code>true</code> for the second parameter.
   */
  public GenomeVariant translateNucleotideVariantToGenomeVariant(
      SingleAlleleNucleotideVariant variant) throws CannotTranslateHGVSVariant {
   ...
   * @return {@link GenomeVariant} resulting from the conversion, possibly annotated with some
   *     warning messages
   * @throws CannotTranslateHGVSVariant in the case of problems such as more than one entry in the
   *     allele of <code>variant</code> or unsupported {@link NucleotideChange}s
   */
  public GenomeVariant translateNucleotideVariantToGenomeVariant(
      SingleAlleleNucleotideVariant variant, boolean autocorrect)
    ...
    if (ntChange instanceof NucleotideSubstitution) {
      result =
          new NucleotideSubstitutionToGenomeVariantTranslationImpl(seqExtractor)
              .run(tm, variant.getSeqType(), (NucleotideSubstitution) ntChange);
    } else if (ntChange instanceof NucleotideDeletion) {
      result =
          new NucleotideDeletionToGenomeVariantTranslationImpl(seqExtractor)
              .run(tm, variant.getSeqType(), (NucleotideDeletion) ntChange);
    } else if (ntChange instanceof NucleotideDuplication) {
      result =
          new NucleotideDuplicationToGenomeVariantTranslationImpl(seqExtractor)
              .run(tm, variant.getSeqType(), (NucleotideDuplication) ntChange);
    } else if (ntChange instanceof NucleotideIndel) {
      result =
          new NucleotideIndelToGenomeVariantTranslationImpl(seqExtractor)
              .run(tm, variant.getSeqType(), (NucleotideIndel) ntChange);
    } else if (ntChange instanceof NucleotideInsertion) {
      result =
          new NucleotideInsertionToGenomeVariantTranslationImpl(seqExtractor)
              .run(tm, variant.getSeqType(), (NucleotideInsertion) ntChange);
    } else if (ntChange instanceof NucleotideInversion) {
      result =
          new NucleotideInversionToGenomeVariantTranslationImpl(seqExtractor)
              .run(tm, variant.getSeqType(), (NucleotideInversion) ntChange);
    } else {
      throw new CannotTranslateHGVSVariant(
