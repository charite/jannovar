...
/**
 * Helper class for converting {@link NucleotidePointLocation}s and {@link NucleotideRange}s to
 * {@link GenomePosition}s and {@link GenomeInterval}s.
 *
...
public class NucleotideLocationConverter {
  ...
  /**
   * Convert {@link NucleotidePointLocation} on a {@link TranscriptModel} to a {@link
   * GenomePosition}
   */
  public GenomePosition translateNucleotidePointLocation(
      TranscriptModel tm, NucleotidePointLocation pos, SequenceType sequenceType)
   ...
   *
   * @param tm {@link TranscriptModel} that the location is relative to
   * @param pos {@link NucleotidePointLocation} to translate
   * @return resulting {@link GenomePosition} of the translation
   * @throws CannotTranslateHGVSVariant in case of problems with the translation
   */
  protected GenomePosition translateCodingNucleotidePointLocation(
      TranscriptModel tm, NucleotidePointLocation pos) throws CannotTranslateHGVSVariant {
    ...
    if (pos.getBasePos() < 0) {
      ...
      // Note that for HGVS, the terminal codon is not part of the CDS whereas it is in
      // TranscriptModel.
    ...
    try {
      return projector
          .transcriptToGenomePos(txPos)
          .withStrand(tm.getStrand())
          .shifted(pos.getOffset());
    } catch (ProjectionException e) {
      throw new CannotTranslateHGVSVariant(
          "could not translate base transcript position "
              + txPos
              + " of "
              + pos.toHGVSString()
              + " to a genome position",
          e);
    }
  }
   ...
   * @throws CannotTranslateHGVSVariant in case of problems with the translation
   */
  protected GenomePosition translateNonCodingNucleotidePointLocation(
      TranscriptModel tm, NucleotidePointLocation pos) throws CannotTranslateHGVSVariant {
    final TranscriptProjectionDecorator projector = new TranscriptProjectionDecorator(tm);
  ...
  public GenomeInterval translateNucleotideRange(
      TranscriptModel tm, NucleotideRange range, SequenceType sequenceType)
      throws CannotTranslateHGVSVariant {
    final GenomePosition firstPos =
        translateNucleotidePointLocation(tm, range.getFirstPos(), sequenceType);
    final GenomePosition lastPos =
        translateNucleotidePointLocation(tm, range.getLastPos(), sequenceType);
