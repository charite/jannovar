...
class NucleotideSubstitutionToGenomeVariantTranslationImpl
  ...
  public NucleotideSubstitutionToGenomeVariantTranslationImpl(
  ...
  public ResultWithWarnings<GenomeVariant> run(
      TranscriptModel tm, SequenceType sequenceType, NucleotideSubstitution ntSub)
    ...
    final GenomeVariant result =
        new GenomeVariant(
                posConverter.translateNucleotidePointLocation(tm, pos, sequenceType),
                fromNT,
                toNT,
                tm.getStrand())
    ...
    if (!refSeq.equals(fromNT)) {
      final GenomeVariant result2 =
          new GenomeVariant(
                  ...
                  tm.getStrand())
              .withStrand(Strand.FWD);
      return ResultWithWarnings.construct(
          result2,
          "Invalid reference nucleotides in "
              + result.toString()
              + ", should be "
              + refSeq
              + ", auto-correcting.");
