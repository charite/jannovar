...
public final class VariantContextAnnotator {
  ...
  public static class Options {
    ...
    /**
     * whether or not to perform shifting towards the 3' end of the transcript (defaults to <code>
     * true</code>)
     */
     ...
     * @param offTargetFilterIntronicSpliceIsOffTarget whether or not to to count non-consensus
     *     intronic splicing as off-target
     */
    public Options(
        boolean oneAnnotationOnly,
        AminoAcidCode code,
  ...
  public VariantContextAnnotator(
      ReferenceDictionary refDict, ImmutableMap<Integer, Chromosome> chromosomeMap) {
  ...
  public VariantContextAnnotator(
      ...
      ImmutableMap<Integer, Chromosome> chromosomeMap,
      Options options) {
    this.refDict = refDict;
    this.chromosomeMap = chromosomeMap;
    this.options = options;
    this.annotator =
        new VariantAnnotator(
            refDict, chromosomeMap, new AnnotationBuilderOptions(options.nt3PrimeShifting, false));
  }
   ...
   *
   * <p>In the case of exceptions, you can use {@link #buildErrorAnnotations} to build an {@link
   * VariantAnnotations} with an error message.
   *
   * @param vc {@link VariantContext} describing the variant
   * @param alleleID numeric identifier of the allele
   * @return {@link GenomeVariant} corresponding to <code>vc</code>, guaranteed to be on {@link
   *     Strand#FWD}.
   * @throws InvalidCoordinatesException in the case that the reference in <code>vc</code> is not
   *     known in {@link #refDict}.
   */
  public GenomeVariant buildGenomeVariant(VariantContext vc, int alleleID)
      throws InvalidCoordinatesException {
    // Catch the case that vc.getChr() is not in ChromosomeMap.identifier2chromosom. This is the
    // case
  ...
  /**
   * Put error annotation messages to a {@link VariantContext} into the ANN field in the INFO
   * column.
   *
   * <p>Previous values are overwritten.
   *
  ...
  public void putErrorAnnotation(VariantContext vc, Set<AnnotationMessage> messages) {
    // TODO(holtgrewe): Do something more elegant way than 15 * "|", needs to be kept in sync with
    // VCFAnnotationData
  ...
  /**
   * Given a {@link VariantContext}, generate one {@link VariantAnnotations} for each alternative
   * allele.
   *
   * <p>Note that in the case of an exception being thrown, you have to add an error annotation
   * yourself to the {@link VariantContext} yourself, e.g. by using {@link #putErrorAnnotation}.
   *
   * @param vc the VCF record to annotate, remains unchanged
   * @return {@link ImmutableList} of {@link VariantAnnotations}s, one for each alternative allele,
   *     in the order of the alternative alleles in <code>vc</code>
   * @throws InvalidCoordinatesException in the case of problems with resolving coordinates
   *     internally, namely building the {@link GenomeVariant} object one one of the returned {@link
   *     VariantAnnotations}s.
   */
  public ImmutableList<VariantAnnotations> buildAnnotations(VariantContext vc)
      throws InvalidCoordinatesException {
    LOGGER.trace("building annotation lists for {}", new Object[] {vc});

    ImmutableList.Builder<VariantAnnotations> builder =
        new ImmutableList.Builder<VariantAnnotations>();
    ...
    for (int alleleID = 0; alleleID < vc.getAlternateAlleles().size(); ++alleleID) {
      if (!annos.get(alleleID).getAnnotations().isEmpty()) {
        for (Annotation ann : annos.get(alleleID).getAnnotations()) {
          boolean offTargetInThis =
              ann.getEffects().stream()
                  .allMatch(
                      e ->
                          e.isOffExome(
                              options.offTargetFilterUtrIsOffTarget,
          ...
          if (!options.oneAnnotationOnly || annotations.isEmpty()) {
            ...
            annotations.add(
                ann.toVCFAnnoString(alt, options.escapeAnnField, options.aminoAcidCode));
          }
        }
      }
    }
   ...
   * @return VariantAnnotations having the message set to {@link
   *     AnnotationMessage#ERROR_PROBLEM_DURING_ANNOTATION}.
   */
  public VariantAnnotations buildErrorAnnotations(GenomeVariant change) {
    return new VariantAnnotations(
        change,
        ImmutableList.of(
            new Annotation(ImmutableList.of(AnnotationMessage.ERROR_PROBLEM_DURING_ANNOTATION))));
  }
}
