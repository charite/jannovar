 ...
 *
 * <p>Part of the Jannovar-HTSJDK bridge.
 *
...
public final class VariantContextWriterConstructionHelper {
  ...
  /**
   * Return a new {@link VariantContextWriter} that uses the header from <code>reader</code> but has
   * the header extended header through {@link #extendHeaderFields}.
   *
   * @param header the VCF header to use for the construction
   * @param outStream {@link OutputStream} to write to
   * @param additionalHeaderLines additional {@link VCFHeaderLine}s to add
   * @return A correct writer for variantContexts
   */
  public static VariantContextWriter openVariantContextWriter(
      VCFHeader header, OutputStream outStream, Collection<VCFHeaderLine> additionalHeaderLines) {
   ...
   */
  public static VariantContextWriter openVariantContextWriter(
      VCFHeader header, String fileName, Collection<VCFHeaderLine> additionalHeaderLines) {
   ...
   * @param fileName path to output file
   * @param additionalHeaderLines additional {@link VCFHeaderLine}s to add
   * @param generateIndex whether or not to generate an index
   * @return A correct writer for variantContexts
   */
  public static VariantContextWriter openVariantContextWriter(
      VCFHeader header,
  ...
  /**
   * Common parts of {@link VariantContextWriterBuilder} creation for the openVariantContextWriter
   * functions.
   */
  ...
  public static VCFHeader extendHeaderFields(VCFHeader header) {
    ...
    VCFInfoHeaderLine annLine =
        new VCFInfoHeaderLine(
            "ANN", 1, VCFHeaderLineType.String, Annotation.VCF_ANN_DESCRIPTION_STRING);
