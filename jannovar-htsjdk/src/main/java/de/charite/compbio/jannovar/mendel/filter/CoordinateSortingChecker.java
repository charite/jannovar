 ...
 *
 * <p>Since VCF files do not have to provide a reference name dictionary in their header, validating
 * the sort order of the chromosomes is tricky. Instead, we check that there is no run of VCF
 * records for a chromosome, something on a different chromsome, and then a previous one again.
 * Within one chromosome, we check by change begin position.
 *
 * <p>In case of problems, {@link #put} throws a {@link VariantContextFilterException}.
 *
...
public class CoordinateSortingChecker implements VariantContextProcessor {
  ...
  private final HashSet<String> seenChromosomes = new HashSet<String>();
