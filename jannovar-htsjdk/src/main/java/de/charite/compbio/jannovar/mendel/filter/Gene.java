 ...
 *
 * <p>Genes are identified by their name, {@link #equals} and {@link #hashCode} only consider the
 * field {@link #name}!
 */
class Gene {
  ...
  public Gene(
      ReferenceDictionary refDict, String name, ImmutableList<TranscriptModel> transcripts) {
  ...
  /**
   * @return {@link GenomeInterval} of this gene (smallest begin and largest end position of all
   *     transcripts).
   */
  ...
  private GenomeInterval mergeRegions(GenomeInterval lhs, GenomeInterval rhs) {
    ...
    return new GenomeInterval(
        lhs.getGenomeBeginPos().getRefDict(),
        Strand.FWD,
        lhs.getGenomeBeginPos().getChr(),
        Math.min(lhs.getBeginPos(), rhs.getBeginPos()),
        Math.max(lhs.getEndPos(), rhs.getEndPos()));
  }
