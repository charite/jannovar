...
public class BuildExampleJannovarDB {
  ...
  private static void build(String name)
    ...
    ResourceUtils.copyResourceToFile(
        "/ex_" + name + "/transcript.gff3", new File(tmpDir + "/transcript.gff3"));
    ResourceUtils.copyResourceToFile(
        "/ex_" + name + "/hgnc_complete_set.txt", new File(tmpDir + "/hgnc_complete_set.txt"));
    ...
    RefSeqParser parser =
        new RefSeqParser(refDict, tmpDir.toString(), new ArrayList<String>(), ini.get("dummy"));
