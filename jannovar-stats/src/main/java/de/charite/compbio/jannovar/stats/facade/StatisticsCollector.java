...
/**
 * Facade class for collecting statistics from a {@link VariantContext} and a list of {@link
 * VariantAnnotations} objects.
 *
...
public class StatisticsCollector {
  ...
  /**
   * Per-sample statistics, <code>null</code> codes for statistics on the variants regardless of
   * genotype (i.e., het/hom will all be 0, for example).
   */
  ...
  public StatisticsCollector(Collection<String> sampleNames) {
    ...
    for (String name : sampleNames) perSampleStats.put(name, new Statistics());
  }
  ...
  public void put(VariantContext vc, List<VariantAnnotations> alleleAnnotations) {
    ...
    for (String sampleName : sampleNames) {
      ...
      if (gt.isHomRef() || gt.isNoCall()) continue; // skip non-alternative genotypes
  ...
  private void putContig(VariantContext vc, String sampleName) {
    ...
    if (sampleName == null) {
      ...
      if (!gt.isHomRef() && !gt.isNoCall()) stats.putContig(vc.getContig());
    }
  }
  ...
  private void putAltAlleleCount(VariantContext vc, String sampleName) {
    ...
    if (sampleName == null) {
      ...
      for (Allele a : gt.getAlleles()) {
        ...
        if (!seen.contains(idx) && idx != 0) count += 1;
        seen.add(idx);
      }
  ...
  private void putFilter(VariantContext vc, String sampleName) {
    final Statistics stats = perSampleStats.get(sampleName);
    for (String ft : vc.getFilters()) stats.putFilter(ft);
  }

  private void putVariantEffect(String sampleName, VariantAnnotations alleleAnno) {
    final Statistics stats = perSampleStats.get(sampleName);
    if (alleleAnno != null
        && alleleAnno.getHighestImpactAnnotation() != null
  ...
  private void putGenomeRegion(String sampleName, VariantAnnotations alleleAnno) {
    ...
    if (alleleAnno.getHighestImpactAnnotation() == null
        || alleleAnno.getHighestImpactAnnotation().getEffects() == null) return;
    final ImmutableSet<VariantEffect> effects =
        alleleAnno.getHighestImpactAnnotation().getEffects();
    final ImmutableSortedSet<VariantEffect> codingEffects =
    ...
    final ImmutableSortedSet<VariantEffect> intronicEffects =
        ImmutableSortedSet.of(
            ...
            VariantEffect.INTRON_VARIANT);
    final ImmutableSortedSet<VariantEffect> utr5Effects =
        ImmutableSortedSet.of(
    ...
    final ImmutableSortedSet<VariantEffect> utr3Effects =
        ImmutableSortedSet.of(
            VariantEffect.THREE_PRIME_UTR_EXON_VARIANT, VariantEffect.THREE_PRIME_UTR_TRUNCATION);
    final ImmutableSortedSet<VariantEffect> upstreamEffects =
        ImmutableSortedSet.of(VariantEffect.UPSTREAM_GENE_VARIANT);
    final ImmutableSortedSet<VariantEffect> downstreamEffects =
        ImmutableSortedSet.of(VariantEffect.DOWNSTREAM_GENE_VARIANT);
    final ImmutableSortedSet<VariantEffect> intergenicEffects =
        ImmutableSortedSet.of(VariantEffect.INTERGENIC_VARIANT);
  ...
  private void putTsTv(VariantContext vc, String sampleName, int alleleIdx) {
    ...
    if (ref.length() == 1 && !alt.isSymbolic() && alt.length() == 1) {
      ...
      if (isTransition(refStr, altStr)) stats.putTsTv(TsTv.TS);
      else if (isTransversion(refStr, altStr)) stats.putTsTv(TsTv.TV);
    }
  }
