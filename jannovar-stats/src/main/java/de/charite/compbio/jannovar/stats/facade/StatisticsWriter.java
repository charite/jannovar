...
public class StatisticsWriter implements AutoCloseable {
  ...
  public StatisticsWriter(StatisticsCollector statsCollector, File targetPath)
  ...
  private void writeTsTvCount() {
    ...
    for (TsTv tsTv : TsTv.values()) {
      ...
      arr.add(
          Integer.toString(
              statsCollector.getPerSampleStats().get(null).getTsTvCount().getOrDefault(tsTv, 0)));

      for (String name : statsCollector.getSampleNames()) {
        arr.add(
            Integer.toString(
                statsCollector.getPerSampleStats().get(name).getTsTvCount().getOrDefault(tsTv, 0)));
      }
      writer.println(Joiner.on('\t').join(arr));
    }
  }
