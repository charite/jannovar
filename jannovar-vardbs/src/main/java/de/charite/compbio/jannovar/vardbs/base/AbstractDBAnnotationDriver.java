...
public abstract class AbstractDBAnnotationDriver<RecordType> implements DBAnnotationDriver {
   ...
   *
   * @param variantProvider {@link DatabaseVariantContextProvider} for querying database for {@link
   *     VariantContext} objects describing annotations from database.
   * @param vcfPath Path to VCF file with dbSNP.
   * @param options configuration
   * @param vcToRecord converter from {@link VariantContext} to record type
   * @throws JannovarVarDBException on problems loading the reference FASTA/FAI file or incompatible
   *     dbSNP version
   */
  public AbstractDBAnnotationDriver(
      DatabaseVariantContextProvider variantProvider,
      String fastaPath,
  ...
  public VariantContext annotateVariantContext(VariantContext obsVC) {
    try (CloseableIterator<VariantContext> iter =
        variantProvider.query(obsVC.getContig(), obsVC.getStart() - 1, obsVC.getEnd())) {
      ...
      HashMap<Integer, AnnotatingRecord<RecordType>> dbRecordsMatch =
          buildAnnotatingDBRecordsWrapper(genotypeMatches, true);
      HashMap<Integer, AnnotatingRecord<RecordType>> dbRecordsOverlap =
          buildAnnotatingDBRecordsWrapper(positionOverlaps, false);
   ...
   *
   * <p>For SNVs, there should only be one value in the value set at which all alleles point to for
   * most cases. The selection of the record for each observed allele is delegated to the subclass'
   * {@link #pickAnnotatingDBRecords}.
   *
   * @param genotypeMatches List of {@link GenotypeMatch} objects to build the annotating database
   *     records from
   ...
   *
   * <p>There can be more than one database record, for example in the case that a SNV is squished
   * together with an indel.
   *
   * @param vc The {@link VariantContext} to annotate
   * @param dbRecordMatches Map from alternative allele index to annotating <code>RecordType</code>
   *     with matching allele
   * @param dbRecordOverlaps Map from alternative allele index to annotating <code>RecordType</code>
   *     with overlapping positions
   */
  protected abstract VariantContext annotateWithDBRecords(
      VariantContext vc,
