 ...
 *
 * <p>This class is an implementation detail and not part of the public interface.
 *
...
public final class AlleleMatcher {
   ...
   *
   * <p>Indels will be left-shifted and normalized when necessary
   *
   ...
   * genotype
   *
   * <p>In the end, all genotypes will be matched regardless of matching alleles, such that later
   * the "best" (e.g., the highest frequency one) can be used for annotating a variant.
   *
   * @param obsVC {@link VariantContext} describing the observed variant
   * @param dbVC {@link VariantContext} describing the database variant
   * @return {@link Collection} of {@link GenotypeMatch}es for the two variants
   ...
   *
   * <p>This will generate one {@link VariantDescription} for each alternative allele in <code>vcf
   * </code>.
   *
    ...
    for (int i = 1; i < vc.getNAlleles(); ++i) {
      VariantDescription vd =
          new VariantDescription(
              vc.getContig(),
              vc.getStart() - 1,
              vc.getAlleles().get(0).getBaseString(),
