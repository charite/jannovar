...
public class ClinVarAnnotationDriver implements DBAnnotationDriver {
  ...
  public VariantContext annotateVariantContext(VariantContext obsVC) {
    try (CloseableIterator<VariantContext> iter =
        vcfReader.query(obsVC.getContig(), obsVC.getStart(), obsVC.getEnd())) {
      // Fetch all overlapping and matching genotypes from database and pair them with the correct
      // allele from vc.
   ...
   */
  private VariantContext annotateWithDBRecords(
      VariantContext obsVC,
    ...
    if (options.isReportOverlapping() && !options.isReportOverlappingAsMatching()) {
      ...
      for (GenotypeMatch m : positionOverlaps) {
        final ClinVarRecord clinVarRecord = converter.convert(m.getDBVC());
        overlapMap.putAll(
            m.getObservedAllele(), clinVarRecord.getAnnotations().get(m.getDbAllele()));
      }
  ...
  private ArrayList<String> buildBasicInfo(
      VariantContextBuilder builder, ArrayListMultimap<Integer, ClinVarAnnotation> matchMap) {
  ...
  private ArrayList<String> buildVarInfo(
      VariantContextBuilder builder, ArrayListMultimap<Integer, ClinVarAnnotation> matchMap) {
    ArrayList<String> result = new ArrayList<>();
    for (int alleleNo : matchMap.keySet().stream().sorted().toArray(Integer[]::new)) {
      for (ClinVarAnnotation anno : matchMap.get(alleleNo)) {
  ...
  private ArrayList<String> buildDiseaseInfo(
      VariantContextBuilder builder, ArrayListMultimap<Integer, ClinVarAnnotation> matchMap) {
    ArrayList<String> result = new ArrayList<>();
    for (int alleleNo : matchMap.keySet().stream().sorted().toArray(Integer[]::new)) {
      for (ClinVarAnnotation anno : matchMap.get(alleleNo)) {
