 ...
 */
final class ExacVariantContextToRecordConverter
    implements VariantContextToRecordConverter<ExacRecord> {
  ...
  public ExacRecord convert(VariantContext vc) {
    ...
    for (ExacPopulation pop : ExacPopulation.values()) {
      ...
      // AC
      List<Integer> lst =
          vc.getAttributeAsList("AC_" + pop).stream()
      ...
      // Het
      lst =
          vc.getAttributeAsList("Het_" + pop).stream()
              .map(x -> Integer.parseInt((String) x))
              .collect(Collectors.toList());
      if (!lst.isEmpty()) {
      ...
      // Hom
      lst =
          vc.getAttributeAsList("Hom_" + pop).stream()
              .map(x -> Integer.parseInt((String) x))
              .collect(Collectors.toList());
      if (!lst.isEmpty()) {
      ...
      // Hemi
      lst =
          vc.getAttributeAsList("Hemi_" + pop).stream()
              .map(x -> Integer.parseInt((String) x))
              .collect(Collectors.toList());
      if (!lst.isEmpty()) {
