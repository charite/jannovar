...
public class ThousandGenomesRecord {
  ...
  /**
   * Filters, RF: failed random forest, AC0: Allele count is zero, InbreedingCoeff: InbreedingCoeff
   * < threshold, LCR: in a low-complexity region, SEGDUP: in a segmental duplication region
   */
  private final ImmutableList<String> filter;
  /** POPMAX for each alternative allele */
  private final ImmutableList<String> popmax;

  // Entries of the INFO column

  /** Observed alternative allele counts for each population */
  private final ImmutableSortedMap<ThousandGenomesPopulation, ImmutableList<Integer>> alleleCounts;
  /** Observed alternative allele het counts for each population */
  private final ImmutableSortedMap<ThousandGenomesPopulation, ImmutableList<Integer>>
      alleleHetCounts;
   ...
   */
  private final ImmutableSortedMap<ThousandGenomesPopulation, ImmutableList<Integer>> chromCounts;
  /** Observed alternative allele frequencies for each population */
  private final ImmutableSortedMap<ThousandGenomesPopulation, ImmutableList<Double>>
      alleleFrequencies;

  public ThousandGenomesRecord(
      String chrom,
      int pos,
      String id,
      String ref,
      ...
      List<String> popmax,
      Map<ThousandGenomesPopulation, List<Integer>> alleleCounts,
      Map<ThousandGenomesPopulation, List<Integer>> alleleHetCounts,
      Map<ThousandGenomesPopulation, List<Integer>> alleleHomCounts,
    ...
    ImmutableSortedMap.Builder<ThousandGenomesPopulation, ImmutableList<Integer>> acBuilder =
    ...
    ImmutableSortedMap.Builder<ThousandGenomesPopulation, ImmutableList<Integer>> acHetBuilder =
        ImmutableSortedMap.naturalOrder();
    ...
    ImmutableSortedMap.Builder<ThousandGenomesPopulation, ImmutableList<Integer>> acHomBuilder =
        ImmutableSortedMap.naturalOrder();
    ...
    ImmutableSortedMap.Builder<ThousandGenomesPopulation, ImmutableList<Integer>> acHemiBuilder =
        ImmutableSortedMap.naturalOrder();
    ...
    ImmutableSortedMap.Builder<ThousandGenomesPopulation, ImmutableList<Double>> afBuilder =
        ImmutableSortedMap.naturalOrder();
    for (Entry<ThousandGenomesPopulation, ImmutableList<Integer>> e :
        this.alleleCounts.entrySet()) {
      ...
      for (int x : e.getValue()) {
        int count;
        if (pop == ThousandGenomesPopulation.POPMAX) count = counts.get(i);
        else count = counts.get(0);
        ++i;

        if (count > 0) afs.add((1.0 * x) / count);
        else afs.add(0.0);
      }
  ...
  public ImmutableSortedMap<ThousandGenomesPopulation, ImmutableList<Integer>>
      getAlleleHetCounts() {
  ...
  /**
   * @return Allele count for the given population <code>pop</code>, for each allele, including
   *     reference one
   */
  ...
  /**
   * @return Allele het count for the given population <code>pop</code>, for each allele, including
   *     reference one
   */
  ...
  /**
   * @return Allele hom count for the given population <code>pop</code>, for each allele, including
   *     reference one
   */
  ...
  /**
   * @return Allele hemi count for the given population <code>pop</code>, for each allele, including
   *     reference one
   */
  ...
  /**
   * @return Chromsome count for the given population <code>pop</code>, for each allele, including
   *     the reference one
   */
  ...
  /**
   * @return Alternative allele frequency for the given population, for each allele, including the
   *     reference one
   */
  ...
  /**
   * @return {@link ThousandGenomesPopulation} with highest allele frequency for the given allele
   *     index (0 is first alternative allele)
   */
  public ThousandGenomesPopulation popWithHighestAlleleFreq(int alleleNo) {
    ...
    for (ThousandGenomesPopulation pop : ThousandGenomesPopulation.values()) {
      if (alleleFrequencies.get(pop) != null
          && alleleNo < alleleFrequencies.get(pop).size()
  ...
  public String toString() {
    return "ThousandGenomesRecord [chrom="
        + chrom
        + ", pos="
        + pos
        + ", id="
