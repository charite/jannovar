...
public final class GenericTSVAnnotationDriver implements DBAnnotationDriver {
  ...
  public VariantContext annotateVariantContext(VariantContext vc) {
    ...
    for (int i = 0; i < options.getValueColumnDescriptions().size(); i++) {
      ...
      final GenericTSVValueColumnDescription desc =
          options.getValueColumnDescriptions().get(colName);
      final String refColName = desc.getRefField();
      final GenericTSVValueColumnDescription refDesc =
          options.getValueColumnDescriptions().get(refColName);
  ...
  private Map<Integer, List<VariantContext>> pickDBRecords(
      VariantContext vc, boolean requireGenotypeMatch) {
    final Map<Integer, List<VariantContext>> result = new HashMap<>();
    for (int i = 0; i < vc.getNAlleles(); ++i) {
      result.put(i, new ArrayList<>());
    }

    try (CloseableIterator<VariantContext> it =
        variantProvider.query(vc.getContig(), vc.getStart() - 1, vc.getEnd())) {
      while (it.hasNext()) {
  ...
  private void annotateWith(
      VariantContext vc,
      String infix,
      Map<Integer, List<VariantContext>> dbRecords,
      GenericTSVValueColumnDescription desc,
      GenericTSVValueColumnDescription refDesc,
    ...
    switch (desc.getValueType()) {
      case Character:
        switch (refDesc.getValueType()) {
          case Character:
            this.<Character, Character>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, null, null);
            break;
          case Flag:
            this.<Boolean, Character>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, null, null);
            break;
          case Float:
            this.<Double, Character>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, Double.MIN_VALUE, Double.MAX_VALUE);
            break;
          case Integer:
            this.<Integer, Character>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, Integer.MIN_VALUE, Integer.MAX_VALUE);
            break;
          case String:
          default:
            this.<String, Character>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, null, null);
            break;
        }
        break;
      case Flag:
        switch (refDesc.getValueType()) {
          case Character:
            this.<Character, Boolean>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, null, null);
            break;
          case Flag:
            this.<Boolean, Boolean>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, null, null);
            break;
          case Float:
            this.<Double, Boolean>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, Double.MIN_VALUE, Double.MAX_VALUE);
            break;
          case Integer:
            this.<Integer, Boolean>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, Integer.MIN_VALUE, Integer.MAX_VALUE);
            break;
          case String:
          default:
            this.<String, Boolean>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, null, null);
            break;
        }
        break;
      case Float:
        switch (refDesc.getValueType()) {
          case Character:
            this.<Character, Double>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, null, null);
            break;
          case Flag:
            this.<Boolean, Double>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, null, null);
            break;
          case Float:
            this.<Double, Double>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, Double.MIN_VALUE, Double.MAX_VALUE);
            break;
          case Integer:
            this.<Integer, Double>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, Integer.MIN_VALUE, Integer.MAX_VALUE);
            break;
          case String:
          default:
            this.<String, Double>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, null, null);
            break;
        }
        break;
      case Integer:
        switch (refDesc.getValueType()) {
          case Character:
            this.<Character, Integer>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, null, null);
            break;
          case Flag:
            this.<Boolean, Integer>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, null, null);
            break;
          case Float:
            this.<Double, Integer>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, Double.MIN_VALUE, Double.MAX_VALUE);
            break;
          case Integer:
            this.<Integer, Integer>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, Integer.MIN_VALUE, Integer.MAX_VALUE);
            break;
          case String:
          default:
            this.<String, Integer>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, null, null);
            break;
        }
        break;
      ...
      default:
        switch (refDesc.getValueType()) {
          case Character:
            this.<Character, String>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, null, null);
            break;
          case Flag:
            this.<Boolean, String>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, null, null);
            break;
          case Float:
            this.<Double, String>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, Double.MIN_VALUE, Double.MAX_VALUE);
            break;
          case Integer:
            this.<Integer, String>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, Integer.MIN_VALUE, Integer.MAX_VALUE);
            break;
          case String:
          default:
            this.<String, String>annotateWithImpl(
                vc, infix, dbRecords, desc, refDesc, builder, null, null);
            break;
        }
        break;
  ...
  private <Label extends Comparable<Label>, Value> void annotateWithImpl(
      VariantContext vc,
      String infix,
      Map<Integer, List<VariantContext>> dbRecords,
      GenericTSVValueColumnDescription desc,
    ...
    for (int alleleNo = 0; alleleNo < vc.getNAlleles(); ++alleleNo) {
      ...
      for (VariantContext dbRecord : dbRecords.get(alleleNo)) {
        ...
        if (label == null && minValue != null && maxValue != null) {
          if (refDesc.getAccumulationStrategy() == GenericTSVAccumulationStrategy.CHOOSE_MIN) {
            label = maxValue;
          } else if (refDesc.getAccumulationStrategy()
              == GenericTSVAccumulationStrategy.CHOOSE_MAX) {
            label = minValue;
          }
        }
        labeledValues
            ...
            .add(
                new LabeledValue<Label, Value>(
                    label, (Value) dbRecord.getAttribute(desc.getFieldName())));
      }
    }
      ...
      case String:
        ...
        for (int j = 0; j < vc.getNAlleles(); ++j) {
          if (!labeledValues.get(j).isEmpty()) {
            annotations.set(
                j, labeledValues.get(j).get(0).getValue().toString().replaceAll(" ", "_"));
          }
        }
        break;
      case Float:
            ...
            switch (refDesc.getAccumulationStrategy()) {
              case AVERAGE:
                annotations.set(
                    j,
                    labeledValues.get(j).stream()
                        .mapToDouble(x -> (Integer) x.getValue())
                        .average()
                        .orElse(0.0));
                break;
              case CHOOSE_FIRST:
