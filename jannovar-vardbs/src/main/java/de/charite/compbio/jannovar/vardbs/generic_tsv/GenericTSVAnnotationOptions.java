...
public class GenericTSVAnnotationOptions extends DBAnnotationOptions {
   ...
   *
   * <p>The value must have the format: <code>
  ...
  public static GenericTSVAnnotationOptions parseFrom(String strValue) {
    ...
    if (tokens.length != 13) {
      throw new RuntimeException(
          "Wrong number of tokens tokens in TSV annotation configuration "
              + strValue
              + " expected 13 but was "
              + tokens.length);
    }
    ...
    final GenericTSVAnnotationTarget target =
        ((colRef == 0) || (colAlt == 0))
            ? GenericTSVAnnotationTarget.POSITION
            : GenericTSVAnnotationTarget.VARIANT;

    return new GenericTSVAnnotationOptions(
        true,
        ovlAsIdentical,
        "",
        MultipleMatchBehaviour.BEST_ONLY,
        new File(pathTsvFile),
        target,
        (oneBasedOffset != 0),
        colContig,
        colStart,
        ...
        ImmutableMap.of(
            ...
            new GenericTSVValueColumnDescription(
                colValue,
                VCFHeaderLineType.valueOf(fieldType),
                fieldName,
                fieldDescription,
  ...
  public GenericTSVAnnotationOptions(
      boolean reportOverlapping,
      boolean reportOverlappingAsIdentical,
      String identifierPrefix,
      MultipleMatchBehaviour multiMatchBehaviour,
      File tsvFile,
      GenericTSVAnnotationTarget annotationTarget,
      boolean oneBasedPositions,
      int contigColumnIndex,
      int beginColumnIndex,
      int endColumnIndex,
      int refAlleleColumnIndex,
      int altAlleleColumnIndex,
      boolean refAlleleAnnotated,
  ...
  public String toString() {
    return "GenericTSVAnnotationOptions [tsvFile="
        ...
        + ", endColumnIndex="
        + endColumnIndex
        + ", refAlleleColumnIndex="
        + refAlleleColumnIndex
        + ", altAlleleColumnIndex="
        + altAlleleColumnIndex
        + ", refAlleleAnnotated="
        + refAlleleAnnotated
        + ", columnNames="
        + columnNames
        + ", valueColumnDescriptions="
        + valueColumnDescriptions
        + "]";
  }
}
