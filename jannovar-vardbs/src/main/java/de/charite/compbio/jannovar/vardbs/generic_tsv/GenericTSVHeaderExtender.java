...
public class GenericTSVHeaderExtender extends VCFHeaderExtender {
   ...
   *
   * <p>In the case of annotating a TSV file with REF and ALT columns, annotate reference and
  ...
  private void addHeadersInfixes(VCFHeader header, String prefix, String infix, String note) {
    for (String colName : tsvOptions.getColumnNames()) {
      final GenericTSVValueColumnDescription desc =
          tsvOptions.getValueColumnDescriptions().get(colName);
      final VCFHeaderLineCount count;
      if (tsvOptions.getAltAlleleColumnIndex() > 0
          && tsvOptions.getRefAlleleColumnIndex() > 0
          && options.isReportOverlapping()
          && !options.isReportOverlappingAsMatching()) {
      ...
      final VCFInfoHeaderLine headerLine =
          new VCFInfoHeaderLine(
              prefix + infix + desc.getFieldName(),
              count,
              desc.getValueType(),
