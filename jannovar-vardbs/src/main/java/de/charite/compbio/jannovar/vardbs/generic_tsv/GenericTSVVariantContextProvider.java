 ...
 *
 * <p>Note that there cannot be concurrent queries with the same <code>
 * GenericTSVVariantContextProvider</code> because we currently only shallowly wrap HTSJDK's
 * TabixReader.
 *
  ...
  private class TabixIteratorWrapper implements CloseableIterator<VariantContext> {
    ...
    private VariantContext parseTabixLine(String resultLine) {
      ...
      if (options.getRefAlleleColumnIndex() > 0 && options.getAltAlleleColumnIndex() > 0) {
        builder.alleles(
            tokens[options.getRefAlleleColumnIndex() - 1],
                    ...
                    .map(
                        s -> {
                          if (s == null || ".".equals(s)) {
                            ...
                            return (Object)
                                ImmutableList.of("1", "Y", "y", "T", "t", "yes", "true")
                                    .contains(s);
                          }
                        })
      ...
      for (String colName : options.getColumnNames()) {
        ...
        final GenericTSVValueColumnDescription refDesc =
            options.getValueColumnDescriptions().get(desc.getRefField());
        ...
        switch (refDesc.getValueType()) {
          ...
          case Float:
            ...
            if (doubleLabeledValues.isEmpty()) {
              ...
              switch (refDesc.getAccumulationStrategy()) {
                ...
                case CHOOSE_MAX:
                  Collections.sort(doubleLabeledValues);
                  key = (int) doubleLabeledValues.get(doubleLabeledValues.size() - 1).getValue();
                  break;
                case CHOOSE_FIRST:
                case AVERAGE:
                default:
          ...
          case Integer:
            ...
            for (int i = 0; i < colValues.get(refDesc.getFieldName()).size(); ++i) {
              ...
              intLabeledValues.add(
                  new LabeledValue<Integer, Object>(
            ...
            if (intLabeledValues.isEmpty()) {
              ...
              switch (refDesc.getAccumulationStrategy()) {
                ...
                case CHOOSE_MAX:
                  Collections.sort(intLabeledValues);
                  key = (int) intLabeledValues.get(intLabeledValues.size() - 1).getValue();
                  break;
                case CHOOSE_FIRST:
                case AVERAGE:
                default:
