...
public class GenericVCFAnnotationDriver extends AbstractDBAnnotationDriver<VariantContext> {
  ...
  public GenericVCFAnnotationDriver(
      String vcfPath, String fastaPath, GenericVCFAnnotationOptions options)
      throws JannovarVarDBException {
    super(
        new VCFReaderVariantProvider(options.getPathVcfFile()),
        fastaPath,
        options,
  ...
  protected HashMap<Integer, AnnotatingRecord<VariantContext>> pickAnnotatingDBRecords(
      ...
      HashMap<GenotypeMatch, AnnotatingRecord<VariantContext>> matchToRecord,
      boolean isMatch) {
  ...
  private void annotate(
      VariantContext vc,
      String infix,
      HashMap<Integer, AnnotatingRecord<VariantContext>> records,
      String fieldName,
      VariantContextBuilder builder) {
    final VCFInfoHeaderLine headerLine =
        genericVcfHeaderExtender.getFileHeader().getInfoHeaderLine(fieldName);
      ...
      for (int i = start; i < end; ++i) {
        ...
        if (records.containsKey(i) && records.get(i).getRecord().hasAttribute(fieldName)) {
          ...
          valueList.add(
              annoRecord
                  .getRecord()
                  .getAttributeAsList(fieldName)
                  .get(annoRecord.getAlleleNo() - offset));
        } else {
          valueList.add(".");
        }
      }
