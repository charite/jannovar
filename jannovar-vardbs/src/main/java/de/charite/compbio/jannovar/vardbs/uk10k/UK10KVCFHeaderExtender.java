...
public class UK10KVCFHeaderExtender extends VCFHeaderExtender {
  ...
  public void addHeadersInfixes(VCFHeader header, String prefix, String infix, String note) {
    ...
    VCFInfoHeaderLine acLine =
        new VCFInfoHeaderLine(
            prefix + infix + "AC",
            VCFHeaderLineCount.A,
            VCFHeaderLineType.Integer,
            "Number of chromosomes showing the given allele in UK10K genomes" + note);
    header.addMetaDataLine(acLine);

    VCFInfoHeaderLine afLine =
        new VCFInfoHeaderLine(
            prefix + infix + "AF",
            VCFHeaderLineCount.A,
            VCFHeaderLineType.Integer,
            "Allele frequency in UK10K genomes" + note);
