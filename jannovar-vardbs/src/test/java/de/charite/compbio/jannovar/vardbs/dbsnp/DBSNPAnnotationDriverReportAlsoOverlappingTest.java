...
public class DBSNPAnnotationDriverReportAlsoOverlappingTest extends DBSNPAnnotationDriverBaseTest {
  ...
  public void testAnnotateVariantContext() throws JannovarVarDBException {
    ...
    Assert.assertEquals(
        "[CAF, COMMON, G5, IDS, OVL_CAF, OVL_COMMON, OVL_G5, OVL_IDS]", keys.toString());

    Assert.assertEquals(
        "[0.97324, 0.02676, 0.0, 0.0]", annotated.getAttributeAsString("CAF", null));
    ...
    Assert.assertEquals(
        "[0.91972, 0.02676, 0.02676, 0.02676]", annotated.getAttributeAsString("OVL_CAF", null));
    ...
    Assert.assertEquals(
        "[rs540538026, rs540538026, rs540538026]", annotated.getAttributeAsString("OVL_IDS", null));
  }
}
