...
public class GenericTSVAnnotationDriverWithDbnsfpReportAlsoOverlappingTest
  ...
  public void testAnnotateVariantContext() throws JannovarVarDBException {
    ...
    Assert.assertEquals(
        "[DBNSFP_AAREF, DBNSFP_HG19POS, DBNSFP_OVL_AAREF, DBNSFP_OVL_HG19POS, "
    ...
    Assert.assertEquals(
        "[., 69119, 69119, 69119]", annotated.getAttributeAsString("DBNSFP_HG19POS", null));
    Assert.assertEquals("[., ., ., .]", annotated.getAttributeAsString("DBNSFP_RS_DBSNP147", null));
    Assert.assertEquals(
        "[., 0.0, 0.0, 0.0]", annotated.getAttributeAsString("DBNSFP_SIFT_SCORE", null));

    Assert.assertEquals("[., L, L, L]", annotated.getAttributeAsString("DBNSFP_OVL_AAREF", null));
    Assert.assertEquals(
        "[., 69119, 69119, 69119]", annotated.getAttributeAsString("DBNSFP_OVL_HG19POS", null));
    Assert.assertEquals(
        "[., ., ., .]", annotated.getAttributeAsString("DBNSFP_OVL_RS_DBSNP147", null));
    Assert.assertEquals(
        "[., 0.0, 0.0, 0.0]", annotated.getAttributeAsString("DBNSFP_OVL_SIFT_SCORE", null));
  }
}
