...
public class Annotation implements Constants, Comparable<Annotation> {
	...
	/**
	 * The position of the variant in the ORF or mRNA, if applicable. This field is used to sort exonic variants. Note
	 * that this variable is used differently for non-coding annotations, in which case it will indicate the distance of
	 * the variant from the nearest exon in nucleotides. The variable is used by two methods, {@link #compareTo}; the
	 * sorting function is meant to be used to sort variants within a gene, and is used right now by the class
	 * {@link exomizer.annotation.AnnotationList AnnotationList}. One such list is made for each Variant. Therefore, if
	 * the sorting method is called for an intergenic variant, they will be sorted by distance to the exon. Since the
	 * sorting also includes priority class, this will not be a problem for variants that have both coding and
	 * non-coding annotations.
	 */
	private int rvarstart;

	/**
	 * The string representing the actual annotation, but not including the gene symbol. For instance, for the complete
	 * annotation "KIAA1751:uc001aim.1:exon18:c.T2287C:p.X763Q", this field would include
	 * "uc001aim.1:exon18:c.T2287C:p.X763Q"
	 */
	private String variantAnnotation = null;

	/**
	 * The gene symbol for this annotation. Thus, for the complete annotation
	 * "KIAA1751:uc001aim.1:exon18:c.T2287C:p.X763Q", this field would contain simply "KIAA1751".
	 * <P>
	 * Note that for intergenic annotations, where two gene symbols are used to identify the upstream and downstream
	 * genes, the symbols are stored directly in the variable {@link #variantAnnotation}. Here, the field is used only
	 * for annotations that can be unambiguously linked to a single gene.
	 */
	 ...
	 * {@link jannovar.common.Constants Constants}, e.g., MISSENSE, 5UTR, etc.
	 *
	...
	/**
	 * This function resets the variant type, and should only be used by the AnnotationList class for certain cases of
	 * resolving precedence, e.g., if there is already a noncoding RNA intronic annotation, and we get a new annotation
	 * for a coding isoform of the same gene.
	 *
	 * @param typ
	 *            {@link VariantType} to be set for this {@link Annotation}
	 */
	...
	/**
	 * Construct a new annotation. This factory method is used instead of a constructor for intergenic annotations where
	 * there is no gene synbol.
	 *
	 * @param annotation
	 *            A complete annotation (without the gene symbol)
	 * @param type
	...
	/**
	 * Client code should set the distance to the nearest exon. This should be used only for INTERGENIC, INTRONIC, and
	...
	/**
	 * Get full annotation with gene symbol. If this Annotation does not have a symbol (e.g., for an intergenic
	 * annotation), then just return the annotation string.<br>
	...
	/**
	 * This method returns the accession number of the transcript associated with this variant (if possible). If there
	 * is no transcript, e.g., for DOWNSTREAM annotations, then it returns the geneSymbol (if possible). If there is no
	 * gene symbol (e.g., for INTERGENIC annotations), it returns "."
	 *
	...
	/**
	 * @param s
	 *            A String representing the new annotation.
	 */
	...
	/**
	 * @param symbol
	 *            The gene symbol for the gene affected by the variant.
	 */
	...
	/**
	 * TODO. Probably this should be refactored, this field should hold some ID number, but we then need to know if it
	 * is from EntrezGene or another source. For now, it is an int that is understood to be an Entrez Gene id, but in
	 * the future we might use this to store ENSEMBL identifiers etc.
	 *
	 * @param id
	 *            The identifier of the gene affected by the variant (e.g., Entrez Gene ID)
	 */
	...
	/**
	 * Checks whether this annotation is equivalent to the annotation being passed as an argument. This may be the case
	 * say for two intronic annotations from different isoforms of a gene where we do not care what the exact position
	 * is.
	 *
	 * @param other
	 *            An annotation to be checked for equivalence.
	...
	public boolean equals(Annotation other) {
		return varType == other.varType
				&& ((geneSymbol != null && geneSymbol.equals(other.geneSymbol)) || (geneSymbol == null && other.geneSymbol == null))
				&& variantAnnotation != null && variantAnnotation.equals(other.variantAnnotation);
	}

	/**
	 * The (private) constructor is intended to be used only by static factory methods. Note that the variable
	 * {@code rvarstart} is set to zero by the constructor. Variants that have a defined position in an ORF assign their
	 * position to this variable, which allows them to be sorted. Other variants (e.g., downstream), which are not
	 * assigned a position, do not need or use this variable.
	 */
	private Annotation() {
		this.rvarstart = 0;
	}
	...
	/**
	 * This method constructs an empty Annotation object. We may be able to switch to simply the constructor, TODO
	...
	/**
	 * This function allows Annotation objects to be sorted. They are first sorted according to the priority level of
	 * the VariantType. If there are multiple Annotations with the same priority level, they are sorted according to
	 * their position in the CDS. For other mutations, rvarstart is 0 and the sorting has no effect.
	 *
	 * @param other
	 *            the {@link Annotation} to be compared with
	 */
	@Override
