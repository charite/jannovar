...
/**
 * This class is intended to provide a static method to generate annotations for duplication mutations. TODO currently
 * the handling of tandem-repeats is not as recommended <br>
 * e.g. these cases:<br>
 * g.7_8dup (or g.7_8dupTG, not g.5_6dup, not g.8_9insTG) denotes a TG duplication in the TG-tandem repeat sequence
 * changing ACTTTGTGCC to ACTTTGTGTGCC <br>
...
public class DuplicationAnnotation {
	...
	public static Annotation getAnnotation(TranscriptModel trmdl, int frame_s, String wtnt3, String var, int startpos, int endpos, int exonNumber) throws AnnotationException {
		 ...
		 */
		int aaVarStartPos = cdsStartPos % 3 == 0 ? (int) Math.floor(cdsStartPos / 3) : (int) Math.floor(cdsStartPos / 3) + 1;
		...
		if (var.length() % 3 == 0) { /* ORF CONSERVING */
			if ((cdsStartPos - 1) % 3 == 0) { /* SIMPLE DUPLICATION OF CODONS, e.g., nucleotide position 4 starts a
				...
				if (var.length() == 3) {
					...
					annot = singleAminoAcidInframeDuplication(trmdl.getName(), exonNumber, canno, wtaaDupStart, aaVarStartPos);
				} else {
					int aaEndPos = aaVarStartPos + (var.length() / 3) - 1; /* last amino acid of duplicated WT seq. */

					String wtAA_after = translator.translateDNA(trmdl.getCdnaSequence().substring(startpos + var.length() - 1, startpos + var.length() + 2));
					if (wtAA_after.equals("*"))
						annot = String.format("%s:exon%d:%s:p.*%dext*%d", trmdl.getName(), exonNumber, canno, aaEndPos + 1, var.length() / 3);
					else
						annot = multipleAminoAcidInframeDuplication(trmdl.getName(), exonNumber, canno, wtaaDupStart, aaVarStartPos, wtaaDupEnd, aaEndPos);
				...
				if (wtaa.equals("*")) { /* Mutation affects the wildtype stop codon */
					...
					if (idx < 0) {
						annot = String.format("%s:exon%d:%s:p.*%d%sext*?", trmdl.getName(), exonNumber, canno, aaVarStartPos, varaa);
					} else {
						annot = String.format("%s:exon%d:%s:p.*%ddelins%s", trmdl.getName(), exonNumber, canno, aaVarStartPos, varaa.substring(0, idx + 1));
					}
				} else {
	...
	private static String shiftedInFrameDuplication(TranscriptModel trmdl, int exonNumber, String cDNAanno, String var, int endpos, int aaVarStartPos, int frame_s) throws AnnotationException {
		...
		if (mutaa.startsWith(wtaa) && (mutaa.indexOf(wtaa, aalen)) > 0) {
			annot = String.format("%s:exon%d:%s:p.%s%ddup", trmdl.getName(), exonNumber, cDNAanno, wtaa, aaVarStartPos);
		} else {
			annot = String.format("%s:exon%d:%s:p.%s%ddelins%s", trmdl.getName(), exonNumber, cDNAanno, wtaa, aaVarStartPos, mutaa);
		}
		return annot;
	}
