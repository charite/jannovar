 ...
 * e.g. these cases:<br>
 * g.7_8dup (or g.7_8dupTG, not g.5_6dup, not g.8_9insTG) denotes a TG duplication in the TG-tandem repeat sequence changing ACTTTGTGCC to ACTTTGTGTGCC <br>
...
public class DuplicationAnnotation {
	 ...
	 * 
	 * @param trmdl
	 *            The transcriptmodel / gene in which the current mutation is
	 *            contained
	 * @param frame_s
	 ...
	 * @param wtnt3_after
	 *            the three nucleotides of the codon following codon affected by
	 *            mutation
	 * @param ref
	 * @param var
	 * @param refvarstart
	 *            The start position of the variant with respect to the CDS of
	 *            the mRNA
	 * @param exonNumber
	 *            Number (one-based) of affected exon.
	 * @return an {@link jannovar.annotation.Annotation Annotation} object
	...
	public static Annotation getAnnotation(TranscriptModel trmdl, int frame_s, String wtnt3,
		...
		int aavarpos =  startPosMutationInCDS % 3 == 0 ? (int) Math.floor(startPosMutationInCDS / 3) : (int) Math.floor(startPosMutationInCDS / 3) +1;
...
//			aavarpos += startPosMutationInCDS % 3;
		...
		String canno;
		if(var.length() == 1)
			canno = String.format("c.%ddup%s", startPosMutationInCDS-1, var); 
		else
			canno = String.format("c.%d_%ddup%s", startPosMutationInCDS-var.length(), startPosMutationInCDS-1, var);
