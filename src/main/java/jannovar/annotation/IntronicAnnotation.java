 ...
 * INTRONIC variants. The main work is in finding the distance to the nearest 5'
 * and 3'
 * 
...
public class IntronicAnnotation {
	 ...
	 * variant. We indicate the distance to the exon boundary exclusive of the
	 * exon but inclusive of the first position of the variant. That is, for a
	 * variant at the indicated position CGGgttag[t/c]g, the distance to the
	 * final (upper-case) G of the previous exon is 6. This makes a standard
	 * intronic variant CGG[g/a]ttagtg have the expected numbering (+1, in this
	 * case).
	 * 
	 * @param trmdl
	 *            The affected transcript
	 * @param k
	 *            Number of exone (zero based numbering) that is 3' to the
	 *            variant on chromosome
	 * @param start
	...
	public static Annotation createIntronicAnnotation(TranscriptModel trmdl, int k, int start, int end) {
		...
		// System.out.println(trmdl.getGeneSymbol() + ":" +accession +
		// ", starnd=" + trmdl.getStrand() + " exon ct = " +trmdl.getExonCount()
		// );
		// System.out.println("K="+k +
		// ", start="+start+", end="+end+" exonend="+exonend+", exonsstart="+exonstart);
	 ...
	 *            The affected transcript
	 * @param k
	 *            exon number (zero based numbering) that is 3' to the variant
	 *            on chromosome
	 * @param start
