...
public class NoncodingAnnotation {
	...
	public static Annotation createNoncodingExonicAnnotation(TranscriptModel trmdl, int rvarstart, String ref, String alt, int exonNumber) {
		...
		if (!(ref.length() == 1 && alt.length() == 1 && !alt.equals("-"))) { // only for non singlenucleotide
																				// substitutions
			while (trmdl.getCdnaSequence().charAt(rvarstart - 1) == trmdl.getCdnaSequence().charAt(rvarstart + ref.length() - 1)) {
				rvarstart++;
		...
		if (alt.equals("-")) {
			/* i.e., deletion  */
			if (ref.length() == 1) {
				annotation = String.format("%s:exon%d:n.%ddel", trmdl.getName(), exonNumber, rvarstart);
			} else {
				int d2 = rvarstart + ref.length() - 1;
				annotation = String.format("%s:exon%d:n.%d_%ddel", trmdl.getName(), exonNumber, rvarstart, d2);
			}
		} else if (ref.equals("-")) {
			/* i.e., insertion */
