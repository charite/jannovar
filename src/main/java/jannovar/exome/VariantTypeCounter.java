...
public class VariantTypeCounter implements Constants {
	...
	/**
	 * The first dimension (rows) represents the samples, the second dimension (columns) represents the variant types.
	 * Thus, countMatrix[i][j] represents the count in sample i for variantType j. Note that the indices for the
	 * variantTypes are stored in the HashMap {@link #variantTypeInd}.
	 */
	...
	/**
	 * The constructor takes a list of all variants found in the VCF file and generates a list of counts, one for each
	 * variant type.
	 *
	 * @param variantList
	 *            List of all variants found in the VCF file.
	 * @throws jannovar.exception.JannovarException
	 */
	public VariantTypeCounter(ArrayList<Variant> variantList) {
		this.n_var_types = VariantType.size();
	...
	/**
	 * Increment the counts for the VariantType represented by this Variant. Note that we extract the GenotypeCall for
	 * all persons in the VCF file, and update the corresponding fields in {@link #countMatrix}.
	 *
	 * @param v
	 *            the {@link Variant}
	 */
	 ...
	 *
	 * @param variantList
	 *            list of {@link Variant}s
	 * @throws JannovarException
	 */
	public void countVariants(ArrayList<Variant> variantList) {
	...
	/**
	 * We store the indices of the VariantTypes in the HashMap {@link #variantTypeInd}. For instance, FS_INSERTION might
	 * have the index 5. This function initializes that HashMap.
	 */
	...
	/**
	 * This class implements an iterator over VariantType objects. The order is guaranteed to be the prioritized order
	 * as defined in the {@link VariantType} class of Jannovar. This is intended to be use by client code to get a list
	 * of VariantTypes in order that will allow HTML rows of variant counts, together with
	...
	// TODO(holtgrem): Should we implement Iterable<T>? Then this would become iterator().
	public Iterator<VariantType> getVariantTypeIterator() {
		return new VariantTypeIterator();
	}

	/**
	 * Returns the number of counts each person in the VCF file has for a specific VariantType (such as MISSENSE, or
	 * UTR3). The order of the entries is the same as the order of entries in the VCF file.
	 *
	 * @param vt
	 *            the {@link VariantType}
