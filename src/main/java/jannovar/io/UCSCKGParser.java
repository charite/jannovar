...
public final class UCSCKGParser extends TranscriptDataParser implements Constants {
	...
	public void parseUCSCFiles() {
		...
		try {
			...
			System.err.println("UCSCKGParser.java: Error with file input");
			System.err.println(kge.toString());
			System.exit(1);
		}
	}
	...
	private void parseKnownGeneMrna(String mrnapath, boolean isGzip) throws KGParseException {
		...
		try {
			...
			br.close();
			System.err.println(String.format("[INFO] Found %d transcript models from UCSC KnownGenes resource, %d of which had sequences", foundSequence, (foundSequence - kgWithNoSequence)));
		} catch (FileNotFoundException fnfe) {
