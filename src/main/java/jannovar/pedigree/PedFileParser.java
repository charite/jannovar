...
/**
 * A class to parse standard Pedfiles (pedigree files) as known from linkage. In particular, we expect a pre-makeped
 * format file
 * <ul>
 ...
 * </ul>
 * Note that in our format, we only use 6 columns, and the sixth column contains the disease phenotype data,
 * <ul>
 * <li>2 means affected
...
public class PedFileParser {
	 ...
	 * {@link #parseFile} if parsing is successful.
	 */
	...
	/**
	 * Parse a PED file that has been put into a BufferedReader by client code (one possible use: a tomcat server). The
	 * result is the same as if we passed the file path to the method {@link #parseFile} but is useful in cases where we
	 * have a BufferedReader but not a file on disk.
	 *
	...
	/**
	 * Parse a Person object from a ped file line that has been split into an Array of Strings that should have (at
	 * least) six fields. Ignore anything more than the first six fields. See the documentation for this class for the
	 * structure of the ped file lines.
	 *
	 * @param A
