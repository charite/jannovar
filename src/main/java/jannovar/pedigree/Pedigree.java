...
/**
 * Models a single-family pedigree in an whole-exome sequencing project. Note
 * that the constructor expects to receive a String with the family ID. If an
 * individual is added to the Pedigree with a different family ID, then an
 * exception is thrown.
 * <p>
 * This class is meant to simulate relatively simple pedigrees that can be
 * conceived of as Affecteds, Parents of Affects, and Unaffected Persons. The
 * class is not meant to be used for formal Lander-Green or Elston-Stewart
 * linkage analysis. Instead, it is meant to be used for relatively simple
 * pedigrees commonly analysed in the setting of whole-exome sequencing.
 * <P>
 * The assumptions of this class are as follows:
 * <P>
 * For autosomal dominant pedigrees, all affected persons must share the same
 * heterozygous mutation, and none of the unaffected persons can carry the
 * mutation.
 * <P>
 * For autosomal recessive pedigrees, there are two possibilities
 * <ul>
 * <li>All affected persons share the same homozygous mutation. In this case,
 * both parents must be heterozygous for the mutation. The unaffected persons
 * can be heterozygous or homozygous wildtype.
 * <li>All affected persons are compound heterozygous for the same two
 * mutations. One of the parents is heterozygous for one of the mutations, and
 * the other parent is heterozygous for the other mutation. Unaffecteds can
 * carry up to one of the mutations.
 * </ul>
 * For X chromosomal mutations, we are currently demanding that affected males
 * are called homozygous for the mutation (actually, they are hemizygous). Only
 * X chromosomal recessive mutations can be analyzed.
 * <P>
 * Note that a common problem is the fact that the order of the sample names in
 * the VCF file may be different from that in the PED file. To simplify the
 * analysis, we demand that the two orders be consolidated, meaning that the
 * order of the PED file samples is adjusted to match the order of the VCF file
 * samples using the function {@link #adjustSampleOrderInPedFile}.
 *
...
public class Pedigree {
	...
	/**
	 * The name of the sample in case this pedigree object is being used to
	 * describe a single sample only.
	 */
	 ...
	 * by the disease. Note that it is possible that a parent is also an
	 * affected person (obviously, this is the case for autosomal dominant
	 * diseases). Note that this list is a subset of the persons in
	 * {@link #personList}.
	 */
	private ArrayList<Person> parentList = null;
	/**
	 * A list of all persons in the pedigree who are unaffected and also not
	 * parents of an affected person. This can comprise both siblings of
	 * affected persons and also other types of relative. For autosomal
	 * recessive diseases, we expect that these people can be either
	 * heterozygous or homozygous wildtype. For autosomal dominant diseases,
	 * these people should be homozygous wildtype. Note that this list is a
	 * subset of the persons in {@link #personList}.
	 */
	...
	public static Pedigree constructSingleSamplePedigree(String name) {
		...
		try {
			...
			throw new RuntimeException(
					"Sample in single-sample pedigree was invalid.");
		}
	}
	...
	/**
	 * Returns the PED file data for the nth person in the pedigree. See
	 * {@link jannovar.pedigree.Person#getPEDFileData getPEDFileData}.
	 *
	 * @param n
	 *            number of the person
	 ...
	 * @param n
	 *            number of the person
	 * @return true if the nth person in the PED file is parent of an affected
	 *         child.
	 */
	...
	/**
	 * This function is used to check whether a sample-id is represented in this
	 * pedigree (it can be used to check that a VCF file and a PED file have the
	 * same samples).
	 *
	 * @param name
	...
	/**
	 * It is possible that the order of the names is different in the PED file
	 * and in the VCF file. We use this function to adjust the order of the
	 * samples in the PED file to be the same as in the VCF file, which makes it
	 * easier to visualize and perform the pedigree analysis.
	 *
	 * @param sampleNames
	 *            List of names from the VCF file.
	 * @throws jannovar.exception.PedParseException
	 */
	public void adjustSampleOrderInPedFile(ArrayList<String> sampleNames)
			throws PedParseException {
		if (sampleNames == null) {
		...
		/*
		 * Now check that the names of the samples are identical to the names in
		 * the PED file.
		 */
		if (sampleNames.size() != getPedigreeSize()) {
			String e = String.format(
					"[Pedigree:adjustSampleOrderInPedFile] Error:"
							+ "%n individuals in pedigree but %d "
							+ "individuals in the VCF file",
					sampleNames.size(), getPedigreeSize());
			throw new PedParseException(e);
	}
		...
		/*
		 * If we get here, the names are the same in both files, but their order
		 * may be different.
		 */
		...
		if (newList.size() != this.personList.size()) {
			String e = String.format("Error adjusting sample order."
					...
					+ " the original PED file has %d", newList.size(),
					personList.size());
			throw new PedParseException(e);
		}
	...
	public void addIndividual(Person person) throws PedParseException {
		if (!this.familyID.equals(person.getFamilyID())) {
			String err = String
					.format("Attempt to add person with different family id (%s) to pedigree for family %s",
	...
	/**
	 * Initialize the three lists, {@link #affectedList}, {@link #parentList}
	 * and {@link #unaffectedList}. This will put each
	 * {@link jannovar.pedigree.Person Person} object with a known Disease
	 * status into one of these three categories. Persons with an unknown
	 * Disease status are not included.
	 */
	private void initializeAffectedsParentsSibs() {
		...
		/*
		 * Note that the following code will still work if there are multiple
		 * sets of parents in the pedigree.
		 */
	...
	/**
	 * This method sets links to the Mother and Father for each person in the
	 * pedigree. If there is some error, then the function returns false. If
	 * everything is ok, it returns true.
	 */
	private boolean findParentLinks() throws PedParseException {
		...
		for (Person p : personList) {
			...
			if (fatherID != null) {
				...
				if (father == null) {
					String s = String
							.format("[Pedigree] Could not find father id: %s",
									fatherID);
			...
			if (motherID != null) {
				...
				if (mother == null) {
					String s = String
							.format("[Pedigree] Could not find mother id: %s",
									motherID);
					throw new PedParseException(s);
				}
				p.setMother(mother);
	...
	/**
	 * @return The number of individuals in the PED file (or simply "1" for a
	...
	/**
	 * Returns the number of parents in the pedigree. If there is only a single
	 * sample, returns zero because the assumption is that a single sample is
	 * from an affected.
	 *
	 ...
	 * {@link jannovar.genotype.GenotypeCall GenotypeCall} object passed to this
	 * function is expected to represent all of the variants found in a certain
	 * gene (possibly after filtering for rarity or predicted pathogenicity).
	 * For autosomal dominant inheritance, there must be at least one Variant
	 * that is shared by all affected persons but no unaffected persons in the
	 * pedigree. The samples represented by the
	 * {@link jannovar.genotype.GenotypeCall GenotypeCall} must be in the same
	 * order as the list of Persons contained in this pedigree.
	 *
	 * @param varList
	 *            A list of variants (usually all variants corresponding to one
	 *            gene).
	 * @return <code>true</code> if gene is autosomal dominant inheritance
	 *         compatible
	 */
	public boolean isCompatibleWithAutosomalDominant(ArrayList<Variant> varList) {
		...
		for (Variant v : varList) {
			...
			boolean variantCompatible = true; /*
											 * Is the current variant compatible
											 * with AD?
											 */
			...
			for (int i = 0; i < N; ++i) {
				...
				if (diseaseStatus == Disease.AFFECTED) {
					if (gt == Genotype.HOMOZYGOUS_REF
							|| gt == Genotype.HOMOZYGOUS_ALT) {
					 ...
					 * variant to be compatible with AD. However, we require
					 * that ther is at least one "good" observation.
					 */
			...
			/*
			 * If we get here, we have either examined all members of the
			 * pedigree or have decided that the variant is incompatible in one
			 * person. If any one variant is compatible with AD inheritance,
			 * than the Gene is compatilbe and we can return true without
			 * examining the other variants.
			 */
	 ...
	 *
	 * @param multiGT
	 *            Genotypes of all persons in the pedigree for the current
	 *            variant.
    ...
    /**
	 * This function checks whether at least one of the variants represented by
	 * the list of genotypes is a homozygous ALT variant that is shared by all
	 * of the affecteds. Additionally, both parents (if they are represented in
	 * the pedigree) must be heterozygous. Any unaffected siblings must be
	 * either heterozygous or homozygous ref.
	 *
	 * @param GT
	 *            a List of Genotypes representing all variants seen in some
	 *            gene
	 * @return true if the distribution of variants is compatible with a
	 *         homozygous mutation.
	 */
	public boolean containsCompatibleHomozygousVariant(GenotypeCall GT) {
		...
		} else if (affectedsAreHomozygousALT(GT) && parentsAreHeterozygous(GT)
				&& unaffectedsAreNotHomozygousALT(GT)) {
			/*
			 * i.e., multiple sample VCF compatible with homozygous var and AR
			 * inheritance.
			 */
			return true;
		} else {
	...
	/**
	 * This function checks that none of the unaffecteds have both variants. By
	 * the time we call this function, we already know that all of the affecteds
	 * are heterozygous for both mutations, that each parent is heterozygous for
	 * only one of the mutations, and thus we only need to show that none of the
	 ...
	 * @return true if none of the unaffecteds are compound het for this pair of
	 *         variants.
	 */
	 ...
	 * @return <code>true</code> if gene is autosomal recessive inheritance
	 *         compatible
	 */
	public boolean isCompatibleWithAutosomalRecessiveHomozygous(
			ArrayList<Variant> varList) {
		if (this.isSingleSample) {
			for (Variant v : varList) {
	 ...
	 *         compatible
	 */
	public boolean isCompatibleWithAutosomalRecessiveCompoundHet(
			ArrayList<Variant> varList) {
		if (this.isSingleSample) {
			int n_het = 0;
			for (Variant v : varList) {
		...
		for (Variant v : varList) {
			...
			if (affectedsAreHeterozygous(multiGT)
				...
				if (fatherIsHeterozygous(multiGT))
					...
					System.err
							.println("ERROR: Neither mother nor father het with at least one parent being het");
					System.exit(1);
		}
			}
		}
		/*
		 * When we get here, we have (potentially empty) lists of GenotypeCalls
		 * that are heterozygous in the father or mother. If there is a
		 * combination of maternal and paternal genotypes that could be a valid
		 * compound heterozygous mutation, then return true! The strategy is to
		 * iterate over paternal variants and check all maternal variants for
		 * compatibility.
		 */
	...
	public boolean isCompatibleWithAutosomalRecessive(ArrayList<Variant> varList) {
		...
		for (Variant v : varList) {
			...
			/*
			 * System.out.println("# 1 not hom"); if
			 * (affectedsAreHeterozygous(multiGT))
			 * System.out.println("# 2 affecteds are het"); else
			 * System.out.println("# 2 affecteds are not het");
			 *
			 * if (onlyOneParentIsHeterozygous(multiGT))
			 * System.out.println("# 3 only one paret is het"); else
			 * System.out.println("# 3 not only one parent is het het");
			 *
			 * if (unaffectedsAreNotHomozygousALT(multiGT) )
			 * System.out.println("# 4 unaffects Not Hom"); else
	 ...
	 */
	public boolean isCompatibleWithXChromosomalRecessive(
			ArrayList<Variant> varList) {
		if (varList.isEmpty()) {
			System.out
					.println("[Pedigree.java] Warning: attempt to test zero-length variant list");
			return false;
		}
		/*
		 * First check whether the gene is X-chromosomal, if not, it cannot be X
		 * chromosomally inherited!.
		 */
		...
		for (Variant v : varList) {
			...
			for (Person p : affectedList) {
				...
				if (g != Genotype.HOMOZYGOUS_ALT) {
					...
					break;
					/*
					 * Cannot be disease-causing mutation, an affected male does
					 * not have it.
					 */
				}
				if (!compatible)
			...
			for (Person p : parentList) {
				int i = p.getIndex();
				Genotype g = gc.getGenotypeInIndividualN(i);
				if (p.isMale() && !p.isAffected()
						&& g == Genotype.HOMOZYGOUS_ALT) {
					compatible = false;
					break;
					/*
					 * Cannot be disease-causing mutation, an unaffected father
					 * has it.
					 */
				}
				if (p.isFemale() && g != Genotype.HETEROZYGOUS) {
					...
					break;
					/*
					 * Cannot be disease-causing mutation, mother of patient is
					 * not heterozygous.
					 */
				}
				if (!compatible)
			...
			for (Person p : unaffectedList) {
				...
				if (p.isMale() && g == Genotype.HOMOZYGOUS_ALT) {
					...
					break;
					/*
					 * Cannot be disease-causing mutation, an unaffected brother
					 * has it.
					 */
				}
				if (p.isFemale() && g == Genotype.HOMOZYGOUS_ALT) {
					...
					break;
					/*
					 * Cannot be disease-causing mutation, an unaffected sister
					 * is homozygos.
					 */
				}
			}
	...
	public void debugPrint() {
		System.out.println("Pedigree: " + familyID + " [n="
				+ getNumberOfIndividualsInPedigree() + "]");
		System.out.println(String.format(
				"Parents: n=%d, Affecteds: n=%d, Unaffecteds: n=%d",
	...
	/**
	 * This function checks whether the gene, whose variants are represented in
	 * the list of genotypes passed to the function are compatible with
	 * autosomal recessive inheritance. This is the case if there is a
	 * homozygous variant or there are two heterozygous variants (this is a
	 * candidate for a compound heterozygous mutation).
	 *
	...
	/**
	 * This function checks whether the gene, whose variants are represented in
	 * the list of genotypes passed to the function, has at least one
	 * heterozygous variant compatible with autosomal dominant inheritance or
	 * with X-chromosomeal recessive inheritance. Since this is a single sample,
	 * we just check in the proband.
	 *
	 * @param varList
	 *            list of {@link Variant}s (usually all variants in some gene)
	 * @return <code>true</code> if gene is autosomal dominant inheritance or
	 *         X-chromosomeal recessive inheritance compatible
	 */
