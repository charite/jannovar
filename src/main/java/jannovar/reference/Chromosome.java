...
public class Chromosome {
	...
	public AnnotationList getAnnotationList(int position, String ref, String alt) throws AnnotationException {
		...
		ArrayList<TranscriptModel> candidateTranscripts = itree.search(start, end);
		...
		if ((ref.length() >= 1000 || alt.length() >= 1000)) {
			if (ref.length() >= 1000)
				candidateTranscripts.addAll(itree.searchBigInterval(start, end));
		...
		if (candidateTranscripts.isEmpty()) {
			if (isStructuralVariant)
				...
				createIntergenicAnnotations(start, end, itree.getLeftNeighbor(), itree.getRightNeighbor());
		...
		if (al.getAnnotationList().isEmpty()) {
			String e = String.format("[Jannovar:Chromosome] Error: No annotations produced for %s:g.%d%s>%s",
					getChromosomeName(), position, ref, alt);
